...
module internal UnaryMetadataFieldOps =
    ...
    /// keyed on the field's concretized declaring type. This is the whole of the opcode's
    ...
    /// declaring type's initialiser has been dealt with by the caller. This is the whole of the
    /// opcode's addressing -- the lazy zero-initialisation of an untouched slot and the two
    /// runtime-initialised statics that get sentinels installed here rather than by a `.cctor`
    /// included -- so that a body the runtime synthesises as `ldsflda` (CoreCLR's
    /// `[UnsafeAccessor]` stubs, `vm/unsafeaccessors.cpp`) addresses a static exactly as the opcode
    /// does.
    ///
    /// Class initialisation is deliberately *not* done here: it can suspend the thread, and what
    /// a caller must do about that depends on the frame it is executing under -- the opcode leaves
    /// its program counter naming the `ldsflda`, to be re-executed. Call `loadClass` first.
