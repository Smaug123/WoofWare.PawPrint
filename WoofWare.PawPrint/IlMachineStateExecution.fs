...
module IlMachineStateExecution =
    let getTypeOfObj
        ...
        | EvalStackValue.NullObjectRef ->
            failwith
                "getTypeOfObj received NullObjectRef; caller must synthesize NullReferenceException before type lookup"
    ...
    let rec callMethod
        ...
        match
            if isIntrinsic then
                ...
                None
        with
        | Some (Intrinsics.IntrinsicCallResult.Completed result) -> result
        | Some (Intrinsics.IntrinsicCallResult.RaiseRuntimeException (exceptionTypeInfo, state)) ->
            raiseRuntimeException loggerFactory baseClassTypes exceptionTypeInfo thread state
            |> fst
        | None ->
