...
type AllocatedArray =
    {
        ...
        /// Backing store in row-major order. For an array `T[d0, d1, ..., d{n-1}]` the
        /// element at index `(i0, i1, ..., i{n-1})` lives at offset
        /// `((((i0)*d1) + i1)*d2 + i2)*...*d{n-1} + i{n-1}` in this flat buffer.
