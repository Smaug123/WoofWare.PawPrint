...
module IlMachineThreadState =
        ...
        let newState =
            { state with
                ...
                Kernel =
                    ...
                    // This thread is PawPrint-internal: no guest can name it, so
                    // no guest can call `sched_getcpu` on it. Consuming a
                    // rotation slot here would let the interpreter's own thread
                    // bookkeeping shift which core every subsequently created
                    // guest thread lands on.
                    //
    ...
    /// layout. The array must have rank >= 1; multi-dim arrays with non-zero lower bounds are
    /// not representable here (C# never emits them, and ECMA-335 II.14.2 calls them out
    /// as a separate constructor form).
    ///
    /// `MultiDimArrayAllocation.totalElements dimensionLengths` must be `Ok`. That is a
    /// precondition rather than a checked error for the reason `allocateArray` gives: only the
    /// caller knows which guest exception a violation deserves, and it is the caller that
    /// received the guest's lengths. Violating it is an interpreter bug, and fails loudly here.
