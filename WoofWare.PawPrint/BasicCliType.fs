...
/// Array element size comes from the current heap; string and localloc strides
/// are fixed by the pointer root itself.
type ByteOffsetNormalisationContext =
    private
        {
            ArrayElementSize : ManagedHeapAddress -> int
        }

[<RequireQualifiedAccess>]
module ByteOffsetNormalisationContext =
    let create (arrayElementSize : ManagedHeapAddress -> int) : ByteOffsetNormalisationContext =
        {
    ...
    let nonArrayRootsOnly : ByteOffsetNormalisationContext =
        {
            ArrayElementSize =
                fun arr ->
                    failwith
                        $"byte-offset normalisation for non-array roots unexpectedly needed array element size for %O{arr}"
        }
...
module ManagedPointerSource =
    ...
    /// `cellSizeOf` resolves an array heap address to its stored element byte
    /// size. The public normalisation entry point is `normaliseByteOffset`.
    ...
    let normaliseByteOffset
        ...
        |> normaliseArrayByteOffset context.ArrayElementSize
    ...
    let unsafeAssumeNormalisedForComparison (src : ManagedPointerSource) : NormalisedManagedPointerSource =
        let fixedRootNormalised =
            normaliseByteOffset (ByteOffsetNormalisationContext.create (fun _ -> 0)) src
