...
| `readdir(3)`'s `d_type` for each inode kind, and the fixed position of `.` and `..` (first, in that order) | `sourcesPure/EnumerateSeeded.cs`, which walks the raw shim. Not implied by the `GetFiles`/`GetDirectories` split: `FileSystemEntry` consults `InodeType` only when it is not `DT_UNKNOWN` and falls back to `stat` otherwise |
