...
Measured on macOS 26.6 / APFS and Linux 6.x arm64 (Apple `container`), at the
uids stated. These are the answers real kernels gave; do not re-derive them, and
do not "correct" one from a header without measuring.
...
| rule | Linux | Darwin |
| --- | --- | --- |
| mode masked with `ACCESSPERMS` | no, keeps all twelve bits | **yes**, so no Darwin guest can create a setuid/setgid/sticky file |
