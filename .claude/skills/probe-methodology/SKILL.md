...
**How to apply:** before writing any claim about ordering, "runs once", or that a re-entry point is unreachable, temporarily instrument the suspicious arm and run the test you think exercises it. An unconditional `failwith "PROBE: <arm> really happens here"` only proves the arm is reached on *some* visit — it aborts execution before any later suspend/resume can occur, so it can't distinguish "runs once" from "reruns on resumption". To settle ordering or repeat-visit questions, increment a counter (or `printfn`) on each entry and `failwith` only once the counter shows a second visit, or trace and inspect the log after a normal run. Thirty seconds either way, and it settles both whether the path is live and whether the claim is true.
