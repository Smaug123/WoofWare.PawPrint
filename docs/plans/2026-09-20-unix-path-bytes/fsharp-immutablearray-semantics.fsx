...
let t name f = printfn "%-46s %s" name (try f () with e -> sprintf "THROWS %s" (e.GetType().Name))
...
t "Map of SAME-length keys"      (fun () -> string (Map.containsKey (ia [1uy;2uy]) (Map.ofList [ia [1uy;2uy], (); ia [9uy;9uy], ()])))
t "Map of DIFFERENT-length keys" (fun () -> string (Map.containsKey (ia [1uy]) (Map.ofList [ia [1uy], (); ia [1uy;2uy], ()])))
