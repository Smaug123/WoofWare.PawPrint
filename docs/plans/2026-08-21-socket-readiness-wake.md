...
| V | peer FIN at a registration whose interest the half-closed level misses (`EPOLLET`-only), then a fresh edge elsewhere, then a MOD widening the interest | batch `[c; l2]`, three runs of three (`order8.c`): the FIN **queued the entry despite the interest miss and it kept the FIN's position** — the FIN's wake is unkeyed, exactly as the sixth review round conjectured. (A first version of this probe wrongly "refuted" the conjecture: its post-FIN zero-timeout wait consumed the very candidate being measured — the seventh round caught the artifact.) |
| W | as V for the *connect completion* (IN-only interest missing the completion's OUT) | batch `[c; l2]`, three runs of three (`order9.c`) — the completion's wake is unkeyed too. So the state-change wakes (completion, refusal, reset, FIN) queue every registration and delivery's re-poll filters, while the data-ready queue push stays keyed (row T) |
| X | as V, but with a waiter *parked in `epoll_wait`* when the FIN lands (`order10.c`, timeout expiring before the newer edge) | the waiter returns **0** and the batch reverses: `[l2; c]`, three runs of three — the woken scan re-polled the masked-out candidate, harvested nothing, and being edge-triggered it was consumed before the waiter re-slept, so the later MOD enqueued fresh. **PawPrint does not model this**: the sweep (`Program.fs`, `fireSocketReadiness`) leaves a parked waiter alone when nothing is deliverable, so the stale candidate survives and a later MOD finds it at the FIN's position. A Codex review of the fuzzer branch (2026-08-22) conjectured the row; the fuzzer cannot reach it (timeout-0 waits never park). The fix's shape: when a wake lands on a port with a parked waiter and no deliverable row, consume the pending-but-undeliverable entries in place, without unparking — a separate slice |
