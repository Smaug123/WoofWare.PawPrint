...
So the int32 evaluation-stack slot carries provenance: `EvalStackValue.Int32` holds an `Int32Source`, which is either an ordinary `Verbatim` number or a `NarrowedManagedPointer`. Byrefs whose bit pattern is exactly known — `Null`, and the `Unsafe.AsRef<T>((void*)bits)` placeholder — are values rather than addresses, so `Int32Source.narrowManagedPointer` truncates them to `Verbatim`; the narrowed case is only ever an *unknown* address.

Putting the case *here* rather than smuggling the narrowed pointer into the wider native-int slot is the whole point. A narrowed pointer in a native-int slot would be a lie about the value's width, and the argument for its safety would be a global one — "no operation that could observe the difference is reachable" — which decays silently the moment someone adds a catch-all. With `Int32Source`, the compiler visits every consumer of an int32: none of them can get at a number without saying what to do when there isn't one. Consumers that genuinely need a number call `Int32Source.value`, which names the opcode in its refusal, so a guest that reaches one fails precisely rather than somewhere downstream.

That mask is decided by pairing two halves of the byref model:

- `ManagedPointerSource.tryContainerAlignmentBits` states how many low bits of the *container start* the real runtime guarantees to be zero. Array data is 8-byte aligned, string character data only 4-byte (it begins at object + 12), `malloc` and `localloc` storage 8-byte, and a PE image's section RVAs page-aligned. Object fields, static fields and stack slots make no claim, and neither expose a stable offset. Nor does a `FieldSignatureBlob` PE range: it lives in the metadata `#Blob` heap and carries a placeholder RVA of 0, so its "low bits" belong to no address.
- `tryStableAddressBits` (and its array/string counterpart in `NullaryIlOp`) states the known in-container byte offset.

`TaggedPointerBits.bitAndOffsetFromAlignedBase` then reduces `(base + offset) & mask` to the tagged-pointer decision procedure above, because `base + offset = base' ||| (offset & lowBits)` for the equally admissible base `base' = base + (offset & ~lowBits)`. The mask is answerable when it selects nothing above the guaranteed alignment (the result is those low bits, as a `Verbatim` int32), or when it preserves every bit (the value is unchanged). Aligning *down* — `p & ~7`, whose answer is a different location in the same container — is refused rather than approximated, because PawPrint cannot yet re-express it as a byref.
