...
**Where this lives in code**: `WoofWare.PawPrint/DeterministicMath.fs`, dispatched from the `Math.Pow`, `Math.Cos`, `Math.Sin`, `Math.Sqrt` and `Math.Ceiling` arms of `Intrinsics.fs`. Only those five are implemented so far; `Floor`, `Tan`, `Log`, `Exp` and the rest of `Math.CoreCLR.cs` remain unimplemented and fail loudly, and should join this module rather than being forwarded to the host. (`Math.SinCos` is a different kind of thing and is also unimplemented: it has an IL body of its own, and bottoms out in a separate `SinCos(double, double*, double*)` InternalCall.)

`Math.Sqrt` and `Math.Ceiling` are the exceptions to everything above, and `Math.Sqrt` appears in this section only for the NaN bullet. `squareRoot` is one of the operations IEEE 754 *requires* to be correctly rounded (§5.4.1, not the recommendation of §9.2), the JIT lowers it to a hardware instruction rather than to a libm call, and every platform's instruction obeys — so there is exactly one right answer for every argument and PawPrint returns it. `TestDeterministicMath.fs` asserts bit-for-bit agreement with the host as a property, which is an assertion the transcendental functions cannot make; `sourcesPure/MathSqrt.cs` correspondingly pins irrational roots exactly, where its `Pow`/`Cos`/`Sin` siblings can only assert identities and slack bounds.

`Math.Ceiling` is further out still: `roundToIntegralTowardPositive` is not merely correctly rounded but *exact*, and it generates no NaN of its own, so it cannot diverge in any way at all — not even in the one NaN bullet that `Math.Sqrt` appears here for. Its host-agreement test in `TestDeterministicMath.fs` therefore carries no table of permitted alternatives, unlike its `pow`, `sin` and `sqrt` counterparts, and `sourcesPure/MathCeiling.cs` pins every result exactly, including the negative zero that IEEE 754 requires for an argument strictly between -1 and 0.

Computing these two in-tree anyway is not about changing the answer, then, but about where the guarantee comes from: the interpreter promises the specified result on its own account rather than inheriting it from whatever machine the recording happened on, and the tests get an exact oracle out of it.
