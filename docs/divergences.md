...
**CoreCLR**: `Math.Pow`, `Math.Cos` and their relatives are `[Intrinsic]` + `MethodImplOptions.InternalCall`, with no IL body; the JIT lowers each to a call into the host platform's C library. The answer is therefore whatever the machine's libm returns.
...
**Where this lives in code**: `WoofWare.PawPrint/DeterministicMath.fs`, dispatched from the `Math.Pow` and `Math.Cos` arms of `Intrinsics.fs`. Only those two are implemented so far; `Sin`, `Log`, `Exp` and the rest of `Math.CoreCLR.cs` remain unimplemented and fail loudly, and should join this module rather than being forwarded to the host. `Sin` in particular is the next one a guest hits: `PortableThreadPool`'s hill-climbing controller calls it nine lines after the `Math.Cos` this change implements.

`Math.Sqrt` is the exception to all of the above, and will not belong in this document when it lands: `sqrt` is one of the operations IEEE 754 *requires* to be correctly rounded, every platform implements it as a hardware instruction that is, and so a deterministic in-tree implementation and the host will agree bit-for-bit by construction.
