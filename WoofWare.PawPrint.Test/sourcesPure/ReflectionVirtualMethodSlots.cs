...
// never `GetParameters()`, which has nothing to do with slots and is covered by
// `ReflectionParameterInfo.cs`. Overloads are therefore given *different return types* purely so
// they can be told apart, except for the vararg pair at the end, which differs only in calling
// convention and is read back that way.
