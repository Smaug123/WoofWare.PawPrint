...
//
// PawPrint places the slots by walking the generic definition's base chain with the type variables
// left symbolic (`vtableOfDefinition`), and only *matches* a candidate override against the slots it
// inherited, rebasing each inherited signature into the deriving definition's own variables. A walk
// that instead concretised both signatures first — substituting the declaring types' generic
// arguments before comparing — makes the candidate match both inherited slots at `T = string`, and
// "most derived wins" then reports `A.M` as the survivor instead of `B.M`.
//
// The file covers three shapes, all of which need placement to happen on the definition:
//
//  1. `A3`/`B3`/`C3`: two inherited signatures that are distinct at the definition level and
//     coincide once T is substituted.
//  2. `Ha`/`Hb`/`Hc`: a `new virtual` shadow whose signature mentions the parameter. Here the tie IS
//     genuine — the two inherited slots really do hold the same signature — and most-derived is the
//     right answer, which is what makes (3) the discriminating case.
//  3. `Ka`/`Kb`/`Kc`: the tie that only looks genuine. `Kb<T> : Ka<string>` pins Ka's parameter, so
//     the two inherited signatures are *both* raw `[!0]` and yet denote different things -- a raw
//     `!0` is scoped to the type that wrote it. Measured: .NET reports `Kc`/`Kb`; trusting the
//     syntactic equality of the raw signatures picks the other slot and reports `Kc`/`Ka`.
//
