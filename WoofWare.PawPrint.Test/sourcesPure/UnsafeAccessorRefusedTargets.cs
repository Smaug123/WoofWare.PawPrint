...
//
// The abstract-class case pins the *order*: a missing constructor is reported as a missing
// constructor even when the class is abstract, so the body's refusal comes after the lookup. (The
// body's refusal itself is `sourcesPure/UnsafeAccessorAbstractConstructor.cs`.)
public class TestUnsafeAccessorRefusedTargets
{
    ...
    private static int Run()
    {
        ...
        r = Check<MissingMethodException>(2, () => NewAbstractNoMatch());
        if (r != 0) return r;

        // A `const` has no storage to take the address of.
        r = Check<MissingFieldException>(3, () => ConstantField(null));
        if (r != 0) return r;
