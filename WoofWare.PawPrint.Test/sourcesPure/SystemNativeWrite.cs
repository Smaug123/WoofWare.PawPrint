...
//   * write of a negative byteCount returns -1 with ERANGE, which
//     Common_Write sets before it looks at the descriptor
//   * the largest size the shim can pass is answered like any other
...
class Program
{
    ...
    static unsafe int Main(string[] args)
    {
        ...
        // ERANGE is 34 on Linux and macOS alike.
        Marshal.SetLastSystemError(0);
        if (SystemNative_Write((IntPtr)1, &b, -1) != -1) return 1;
        if (Marshal.GetLastSystemError() != 34) return 6;
        // ...and ahead of the descriptor, which the C never looks up.
        Marshal.SetLastSystemError(0);
        if (SystemNative_Write((IntPtr)(-1), &b, -1) != -1) return 7;
        if (Marshal.GetLastSystemError() != 34) return 8;

        // The largest size the shim can pass, on a descriptor that is not
        // open: EBADF, as for any size. Darwin refuses only counts above
        // INT_MAX, and Linux's range screen from NULL passes this one.
        Marshal.SetLastSystemError(0);
        if (SystemNative_Write((IntPtr)(-1), (byte*)0, int.MaxValue) != -1) return 9;
