...
module TestBinaryArithmetic =
    ...
    let private expectTaggedAddress
        ...
        =
        match actual with
        | EvalStackValue.Int64 (Int64Source.ManagedAddress actual) ->
            actual.Storage |> shouldEqual expectedStorage
            actual.Offset |> shouldEqual expectedOffset
        | other ->
            failwith $"expected tagged int64 managed address %O{expectedStorage} + %d{expectedOffset}, got %O{other}"
    ...
    [<Test>]
    let ``tagged int64 arithmetic accepts nativeint managed pointer operands`` () : unit =
        let state, arr = stateWithIntArray [ 10 ; 20 ; 30 ]
        let ptr = ManagedPointerSource.Byref (ByrefRoot.ArrayElement (arr, 1), [])
        let address = expectManagedAddressProjection state ptr
        let tagged = EvalStackValue.Int64 (Int64Source.ManagedAddress address)
    ...
    [<Test>]
    let ``tagged int64 arithmetic accepts raw managed pointer operands`` () : unit =
        let state, arr = stateWithIntArray [ 10 ; 20 ; 30 ]
        let ptr = ManagedPointerSource.Byref (ByrefRoot.ArrayElement (arr, 1), [])
        let address = expectManagedAddressProjection state ptr
        let tagged = EvalStackValue.Int64 (Int64Source.ManagedAddress address)
    ...
    [<Test>]
    let ``tagged int64 comparisons project nativeint managed pointer operands through state`` () : unit =
        let state, arr = stateWithIntArray [ 10 ; 20 ; 30 ]
        let ptr = ManagedPointerSource.Byref (ByrefRoot.ArrayElement (arr, 1), [])
        let address = expectManagedAddressProjection state ptr
        let tagged = EvalStackValue.Int64 (Int64Source.ManagedAddress address)
    ...
    [<Test>]
    let ``verbatim tagged int64 arithmetic preserves raw high-bit values`` () : unit =
    ...
    [<Test>]
    let ``ordinary signed int64 arithmetic does not accept tagged-only mixed operands`` () : unit =
        let state, arr = stateWithIntArray [ 10 ; 20 ; 30 ]
        let signed = EvalStackValue.ofInt64 1L

        let assertInvalid (left : EvalStackValue) (right : EvalStackValue) : unit =
            let ex =
                Assert.Throws<System.Exception> (fun () -> execute ArithmeticOperation.add state left right |> ignore)

            ex.Message |> shouldContainText "invalid add operation"

        assertInvalid signed (EvalStackValue.Int32 1)
        assertInvalid (EvalStackValue.Int32 1) signed
        assertInvalid signed (EvalStackValue.NativeInt (NativeIntSource.Verbatim 1L))
        assertInvalid signed (arrayPointer arr 0)

    [<Test>]
    let ``ordinary signed int64 comparisons do not accept tagged-only mixed operands`` () : unit =
        let state, arr = stateWithIntArray [ 10 ; 20 ; 30 ]
        let signed = EvalStackValue.ofInt64 1L
        let projection = IlMachineState.tryManagedPointerAddress baseClassTypes state

        let assertInvalid (expectedMessage : string) (compare : unit -> bool) : unit =
            let ex = Assert.Throws<System.Exception> (fun () -> compare () |> ignore)

            ex.Message |> shouldContainText expectedMessage

        assertInvalid "Cgt.un invalid" (fun () -> EvalStackValueComparisons.cgtUn signed (EvalStackValue.Int32 1))

        assertInvalid "Clt.un invalid" (fun () -> EvalStackValueComparisons.cltUn (EvalStackValue.Int32 1) signed)

        assertInvalid "bad ceq" (fun () -> EvalStackValueComparisons.ceq signed (EvalStackValue.Int32 1))

        assertInvalid
            "bad ceq"
            (fun () -> EvalStackValueComparisons.ceqWithManagedPointerProjection projection signed (arrayPointer arr 0))

    [<Test>]
    let ``add ovf refuses raw tagged int64 operands instead of using signed overflow`` () : unit =
        let ex =
        ...
        ex.Message |> shouldContainText "refusing raw unsigned int64 operands"

    [<Test>]
    let ``tagged int64 managed address arithmetic rejects raw negative byte offsets`` () : unit =
    ...
    [<Test>]
    let ``tagged int64 managed address offset overflow fails with managed-address context`` () : unit =
        let storage = Some (ByteStorageIdentity.Array (ManagedHeapAddress 101))
    ...
    [<Test>]
    let ``unsigned tagged int64 managed address comparisons support null checks`` () : unit =
        let storage = Some (ByteStorageIdentity.Array (ManagedHeapAddress 101))
    ...
    [<Test>]
    let ``tagged int64 managed address arithmetic preserves storage identity`` () : unit =
        let mutable arrayCases = 0
        let mutable stringCases = 0
        let mutable localMemoryCases = 0
        let mutable stackLocalCases = 0
        ...
        let property (case : TaggedAddressCase) : unit =
            ...
            let offset = EvalStackValue.ofInt64 case.Step
