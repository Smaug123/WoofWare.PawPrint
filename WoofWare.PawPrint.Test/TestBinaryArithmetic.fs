...
module TestBinaryArithmetic =
    ...
    let ``byte offset helper normalises every byte-addressable root with generated offsets`` () : unit =
        ...
        let property (case : ByteOffsetNormalisationCase) : bool =
            ...
            let context = ByteOffsetNormalisationContext.create (fun _ -> case.ArrayCellSize)
            ...
            raw |> ManagedPointerSource.normaliseByteOffset context |> shouldEqual expected

            smart |> ManagedPointerSource.normaliseByteOffset context |> shouldEqual smart
