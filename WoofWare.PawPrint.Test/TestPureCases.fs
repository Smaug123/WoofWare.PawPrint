...
module TestPureCases =
    ...
    let unimplemented =
        [
            "TypeOpenGenericDefinitionCrossAssembly.cs" // `typeof(List<>)` -- an open generic definition declared in *another* assembly -- named from inside a generic type context, where the enclosing `!0` is bound. The same-assembly twin, `TypeOpenGenericDefinitionInGenericContext.cs`, passes. The two reach different arms of `ldtoken`'s type-token decoding: a type in another assembly arrives as a `TypeReference` token and one in this assembly as a `TypeDefinition`, and only the second yields the open definition. Measured: PawPrint returns 1, meaning `IsGenericTypeDefinition` was false because the token came back closed at the enclosing instantiation; real .NET returns 0. Independent of member-token work -- `main` diverges identically (measured against its own build). Un-parking this is also what makes the `TypeReference` arm's `allowOpenGenericDefinition` flag mutation-killable: while this path is broken, no passing guest observes that flag, and flipping it survives the whole suite. Verified to exit 0 on real .NET.
