...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "Threads.cs" // past `RuntimeHelpers::TryEnsureSufficientExecutionStack` (returns true) and the `Unsafe.As<object?, ExecutionContext?>(ref m_stateObject)` byref shape on async resume (now short-circuited in `readManagedByref` for zero-offset ref-type-over-ObjectRef cells); now blocks downstream at the unimplemented QCall `System.Threading.Thread::YieldInternal() -> bool` (`ThreadNative_YieldThread`), which CoreLib's Task scheduler calls on the spin path
