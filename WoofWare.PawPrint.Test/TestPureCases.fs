...
module TestPureCases =
    ...
    let unimplemented =
        [
            "RawRootWholeStrideReadNextOnly.cs" // `Unsafe.Add(ref p->A, 4)` for a `T*` into stackalloc'd memory where only `p[1]` has been written. The byref is `[Field A; ReinterpretAs int; ByteOffset 16]` over the block's byte 0, and the load resolves `Field A` against the typed cell stored at byte 0, which is none: "TODO: typed read of local memory <stack memory block #0> at byte offset 0 has no typed cell here; needs a byte-view byref shape". The destination does hold a value. Un-park with `StackallocFieldStoreIntoFreshBlock.cs`: it needs a `Field` step off a localloc root resolved without a typed cell stored at the root's offset. Verified to exit 0 on real .NET.
            "RawRootWholeStridePointerEquality.cs" // `Unsafe.AsPointer(ref Unsafe.Add(ref p->A, 4)) == &p[1].A`, with both `p[0]` and `p[1]` written: the same comparison as `RawRootFieldStepAcrossStructs.cs` makes with `Unsafe.AreSame`, made on native ints. The two byrefs are on different roots, one of them with a byte cursor, so only their byte coordinates decide it, and the native-int `ceq` arm (`NativeIntSourceComparison.equalsForCli`) calls `ManagedPointerSource.ceqNormalised`, which cannot compute them: "TODO (CEQ): native-int-wrapped byref compares byrefs on different roots where at least one carries a byte cursor this comparison cannot place within its root". `EvalStackValueComparisons.ceqDeferred` defers only the byref-vs-byref pair to `StorageLocation.resolveCeq`. Un-park when it defers a native-int-wrapped byref pair too. Verified to exit 0 on real .NET.
            "RawRootWholeStrideIdentityUnwritten.cs" // `Unsafe.AreSame(ref Unsafe.Add(ref p->A, 4), ref p[1].A)` for a `T*` into stackalloc'd memory whose `p[1]` has never been written. The first byref is `[Field A; ReinterpretAs int; ByteOffset 16]` over the block's byte 0 and the second `[Field A]` over byte 16, so they are on different roots and comparing them needs each one's byte coordinate; the second's comes from resolving `Field A` against the typed cell stored at byte 16, and there is none. Measured: "TODO (CEQ): Unsafe.AreSame compares byrefs on different roots where at least one carries a byte cursor this comparison cannot place within its root". Un-park when a `Field` step off a localloc or native-heap root can be placed without a typed cell stored at the root's offset -- from the field's declaring type, say -- which `StorageLocation.rootTemplate` cannot do today. `RawRootFieldStepAcrossStructs.cs` covers the same comparison with `p[1]` written. Verified to exit 0 on real .NET.
            "StackallocFieldStoreIntoFreshBlock.cs" // `p->A = 1` through an `S*` into stackalloc'd memory that nothing has written yet. The store is a typed write through `[Field A]` over the block's byte 0, which has to read the struct stored there to update one field of it, and there is none: "TODO: typed read of local memory <stack memory block #0> at byte offset 0 has no typed cell here; needs a byte-view byref shape". Un-park when a `Field` step off a localloc or native-heap root can be resolved without a typed cell stored at the root's offset, the same gap as `RawRootWholeStrideIdentityUnwritten.cs`. Verified to exit 0 on real .NET.
            ...
            "MarshalOffsetOfLayoutClass.cs" // `Marshal.OffsetOf` on a class with `[StructLayout]`, including a field inherited from a layout base class, which CoreCLR answers from the base's own native layout (the QCall looks at the field's enclosing type, not the one the guest named). PawPrint computes a native layout for value types only, so the QCall refuses: "TODO MarshalNative_OffsetOf: unimplemented marshalling case for Program+LayoutClass ...: ... is a class with [StructLayout], and PawPrint does not compute a class's native layout" (measured). `Marshal.SizeOf` of a layout class is refused for the same reason. Un-park when a class's native layout is computed, which is a walk over its base chain: a derived class's fields start at its base's native size, as `DerivedFromSized` checks. Verified to exit 0 on real .NET.
