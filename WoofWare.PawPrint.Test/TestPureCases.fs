...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "AssemblyGetNameSimpleName.cs" // `AssemblyNative_GetSimpleName`, `_GetVersion` and `_GetLocale` are implemented and unit-tested in `TestAssemblyNativeQCalls.fs`; what is parked here is the only managed path a guest can reach them by. `Assembly.GetName()` fills an `AssemblyName` from six QCalls in a row (RuntimeAssembly.cs:112), and PawPrint answers the first three: it now gets past `GetLocale` and stops at the unimplemented `AssemblyNative_GetPublicKey` (`RuntimeAssembly::GetPublicKey(QCallAssembly, ObjectHandleOnStack) -> void`) via `NativeCall.failUnimplemented` (Native/NativeCall.fs:689). `GetFlags` and `GetCodeBase` follow it. Un-park when those land.
