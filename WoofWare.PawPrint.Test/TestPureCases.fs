...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "SpanMemmoveHeapFieldProjectedOverlap.cs" // Heap-rooted analogue of the above: same undecidable-overlap fail-loud, but the struct lives as a field of a class so the byref root is `HeapObjectField`. Folding `Field` projections into a flat byte offset would let this case decide direction correctly; until then this is a deliberate fail-loud diagnostic.
        ]
        |> Set.ofList
