...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "SpanBulkWritePrimitiveLike.cs" // Writing an element through a span byref stores the eval-stack representation rather than the destination cell's storage form, so the write succeeds but the next ordinary read fails: `bool` -> "expected one-byte integer in Ldelem.u1, got: Bool", `nint`/`nuint` -> "expected native int in Ldelem.i", enums -> "expected two-byte integer in Ldelem.i2, got: ValueType ... EnumLike". Exactly the element types whose CliType is primitive-like but distinct from the raw storage form; `char`/`byte`/`int`/`long`/`double`, multi-field structs and reference types are fine (see the passing SpanFill.cs). Predates `Span<T>.Fill` and is not about running BCL IL: the natively-implemented `Span<T>.Clear` corrupts these identically, so each case is driven through both. Plain stelem/ldelem round-trips are unaffected.
