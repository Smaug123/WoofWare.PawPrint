...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "RegexConstruction.cs" // Constructing three `Regex`es in one process. Each construction passes on its own, and `Regex(s)` followed by `Regex(s, NonBacktracking)` passes too; what fails is a *second* `NonBacktracking` construction, because `System.Text.RegularExpressions.Symbolic`'s BDD caches are process-wide, so the second regex is the first to find an entry already present and take a cache *hit*. The hit path is `Dictionary<BDD, ValueTuple`2[]>.TryGetValue`, whose `ldobj !!TValue` concretizes to a structural `ConcreteTypeHandle.OneDimArrayZero`; `executeLdobj` (UnaryMetadataMemoryOps.fs:166) assumes a `Concrete` handle and does `AllConcreteTypes.lookup ... |> Option.get`, which is `None` for every structural handle, so the run dies with FSharp.Core's "The option value was None" naming neither the opcode nor the type. It needs the array/pointer/byref arms of the classification `Intrinsics.isValueTypeHandle` (Intrinsics.fs:2133) already spells out. Nothing here is Regex-specific — any generic dictionary whose value type is an array reaches it. Behind that sits a second blocker, measured by giving `ldobj` the array arm locally: the run then reaches a `Buffer.Memmove` (Native/NativeBuffer.fs:149) over the `Entry[]` of a `Dictionary` keyed by `ValueTuple<int, BDD>`, and `readArrayBytesAs` (IlMachineManagedByref.fs:728) refuses a byte view over an element whose value type contains object references — the same byte-imageless-value-type family as the other parked write-path cases. Un-park when both land.
