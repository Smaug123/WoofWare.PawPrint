...
module TestPureCases =
    ...
    let unimplemented =
        [
            "AreSameProjectionCrossesArrayElement.cs" // `Unsafe.AreSame` between a byte view of `a[0].Y` advanced 4 bytes and a byte view of `a[1]`, for `struct Pair { int X; int Y }`. Those are one address and real .NET says so. The *comparison* no longer blocks this: byref `ceq` now defers undecidable pairs to `StorageLocation.resolveCeq`, which resolves both sides to one `ByteStorageIdentity.Array` and compares offsets — its three siblings un-parked on exactly that change. Measured on top of it, this guest now fails strictly earlier, at `IL offset 71`, in the byte-view *read* that builds the operand: "byte-view read at offset 8 for 1 bytes does not fit in single primitive cell of size 8" for `<field Y of element 0 of array> as System.Byte + 4 bytes`. That is the "a byte cursor may not leave the cell it started in" gap — the same family as `GCMemoryInfoSpanProperties.cs` (issue #729), and nothing to do with byref identity. So its old note ("un-park with the other AreSame guests") is now wrong: its siblings have landed and this one has a second, unrelated dependency. Un-park when a byte view can cross out of its originating cell.
