...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "Threads.cs" // past the `RuntimeHelpers::TryEnsureSufficientExecutionStack` InternalCall (now stubbed to always return true in `NativeRuntimeHelpers.tryExecute`); now blocks downstream in `IlMachineManagedByref.validateByteAddressableCell` — async resume reads `m_stateObject` (an object reference) as `System.Threading.ExecutionContext` through a single-cell byref byte view, which the byref machinery refuses because it can't model byte-addressable views over object references
