...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "DefaultSpanSequenceEqual.cs" // A `default` span carries a null byref in `_reference`, and the bitwise-equatable path of `SequenceEqual`/`StartsWith`/`EndsWith` reinterprets `MemoryMarshal.GetReference(span)` through `Unsafe.As<T, byte>` before it looks at the length. Appending that reinterpret projection to a null managed pointer fails with "cannot project from null managed pointer" (`ManagedPointerSource.fs:477`), though the real runtime never dereferences it at length 0. Predates the prefix/suffix work — `SequenceEqual` alone reproduces it. Fixing the null-byref projection should unblock all three at once.
