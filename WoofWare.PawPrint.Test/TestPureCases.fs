...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "MarshalStructureToPtrDecimalField.cs" // `StructMarshalStub.isBlittableField` rejects Decimal fields (CoreCLR routes them through `NFT_DECIMAL` stub synthesis because native `DECIMAL` is 8-byte aligned while managed `Decimal` is 4-byte aligned), so the struct reaches the has-layout-non-blittable arm and `tryComputePlan` then declines it. The stub machinery a Decimal field would run on now exists (it is what marshals a `DateTime` field); two Decimal-specific gaps remain. (1) `Marshal.SizeOf&lt;{int; decimal}&gt;()` returns 20 instead of 24 because the marshal-layout walk in `CliValueType.TryComputeMarshalLayout` doesn't bump Decimal's field alignment to 8 — so the placement it hands the stub is wrong before the stub runs. (2) There is no `StructMarshalFieldKind` for it: unlike `OADate`, which is one managed field to one native range, native `DECIMAL` reorders its 32-bit members relative to managed `System.Decimal`, so it may need several native writes per managed field — which is a change to the step's shape, not just a new case.
