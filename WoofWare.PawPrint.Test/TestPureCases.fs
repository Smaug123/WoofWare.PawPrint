...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "AssemblyGetNameSimpleName.cs" // `AssemblyNative_GetSimpleName` and `_GetVersion` are implemented and unit-tested in `TestAssemblyNativeQCalls.fs`; what is parked here is the only managed path a guest can reach them by. `Assembly.GetName()` fills an `AssemblyName` from six QCalls in a row (RuntimeAssembly.cs:112), and PawPrint answers the first two: it now gets past `GetVersion` and stops at the unimplemented `AssemblyNative_GetLocale` (`RuntimeAssembly::GetLocale(QCallAssembly, StringHandleOnStack) -> void`) via `NativeCall.failUnimplemented` (Native/NativeCall.fs:689). `GetPublicKey`, `GetFlags` and `GetCodeBase` follow it. Un-park when those land.
