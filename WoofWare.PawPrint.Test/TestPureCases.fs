...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "ReflectionOpenGenericStructConstructors.cs" // `typeof(SBox<>).GetConstructors()` for a *value-type* definition. The introduced-method walk serves it -- the reference-type sibling `ReflectionOpenGenericConstructors.cs` is active and covers that -- and it dies one step later, in `PopulateConstructors`' call to `RuntimeMethodHandle.GetStubIfNeeded`. A value-type declaring type makes `stubOutcome` say `Rebind` (`needsStub` is true for `IsValueType`, because CoreCLR wants the unboxing stub), and rebinding needs the declaring type's generic arguments as a substitution context; for a definition those are its own type variables, which `ConcreteTypeHandle` cannot express. Measured: "RuntimeMethodHandle.GetStubIfNeededSlow: rebinding onto open generic definition ... is not supported". Note the instantiation is *empty* here, which is why the class case never reaches it: a class definition's ctor takes `GetStubIfNeeded`'s fast path and returns the original handle. Un-park when a method handle can carry a formal (definition-level) type context -- the same capability `RuntimeTypeHandle.GetNumVirtuals` lacks for an open definition. Verified to exit 0 on real .NET.
