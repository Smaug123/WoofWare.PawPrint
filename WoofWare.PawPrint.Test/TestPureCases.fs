...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "CovariantReturnDispatchMultiHop.cs" // A covariant-return chain that narrows at *every* level, where Roslyn names the *immediate parent* in each MethodImpl (measured: `L1`'s declaration is `L0::F`, `L2`'s is `L1::F`), so no MethodImpl names `L0::F` from `L2` and yet `((L0)l2).F()` must reach `L2.F`. CoreCLR carries it up by *unifying* slots to a fixed point (`SetupMethodTable2`, methodtablebuilder.cpp:11344-11381, whose comment records that MethodImpl on a class came to mean "unify the slots of A and B" and that compilers rely on it); one pass leaves `L0`'s slot holding `L1.F`. That is what PawPrint answers: the walk rejects `L2.F` as `newslot`, finds no MethodImpl at `L2` declaring the call site, and takes `L1`'s, which does. Measured from a debug trace -- the call resolves to `L1::F`, so a `Mid` comes back and check 1 fails. A silently wrong body from plain C# with no generics, and the shape that discriminates a fixed point from a single pass. The single-level shapes that already work are the active `CovariantReturnDispatchChain.cs`, so a regression there is distinguishable from this gap. Un-park with the other two dispatch cases, when dispatch is keyed off the slot table and slot contents are unified. Verified to exit 0 on real .NET.
            "VirtualDispatchNewSlotShadow.cs" // A `callvirt` naming a base slot that a `new virtual` shadow displaced, with no generics anywhere. `tryResolveVirtualImplementationForSlot` picks the body by walking the receiver's type chain and comparing *signatures* against the call target's (`signatureMatchesTarget`, IlMachineStateExecution.fs), answering with the most-derived non-NewSlot match. Its NewSlot screen (`meth.IsNewSlot && not (MethodInfo.sameDeclaredMethod meth methodToCall)`) rejects the shadow itself, but cannot reject an *override of* the shadow: that override carries no NewSlot bit, and nothing in the walk records which slot it took. So a call spelled `Na::M` is answered by `Nc.M`, which overrode Nb's slot. Measured: PawPrint returns 33 and 43 where real .NET returns 31 and 41 -- a silently wrong body rather than a crash, reachable from plain C# with no generics. `NativeRuntimeTypeHelpers.slotTableOfClosed` already computes the layout reflection reads, so the fix is to resolve the call site to a slot index and take that slot's occupant instead of matching signatures. Un-park when dispatch is keyed off the slot table. Verified to exit 0 on real .NET.
            "VirtualDispatchGenericDefinitionSlots.cs" // The dispatch counterpart of `ReflectionVirtualSlotsGenericDefinitionLayout.cs`: which method answers a `callvirt` is decided by slot, and slots are laid out on the generic definition. Two causes compound here. `signatureMatchesTarget` compares signatures *concretised* at each declaring type's arguments, so at `T = string` the candidate `C3<T>.M(T)` matches a call site spelled `B3<string>::M(string)` and answers it, where B3's slot still holds `B3.M`; and as in [VirtualDispatchNewSlotShadow.cs] an override of a `new virtual` shadow is indistinguishable from an override of the slot the call site names. Measured PawPrint: 3 where .NET gives 2 (check 2), 13 where .NET gives 12 (check 4), and 23 where .NET gives 21 (check 5). Checks 1, 3 and 6 -- the calls through the slots those overrides really did take -- pass, so a failure is attributable to the choice of slot rather than to override lookup as a whole. Un-park when dispatch is keyed off a slot table laid out on the generic definition. Verified to exit 0 on real .NET.
