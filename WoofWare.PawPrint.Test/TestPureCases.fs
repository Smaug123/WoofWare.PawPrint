...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "InlineArrayFieldWriteAtLaterSlot.cs" // `buffer[k].Tag = v` for `k > 0`. Slot 0 works (`InlineArrayFieldWriteThroughIndex.cs`); a later slot puts a `ByteOffset` between the reinterpret and the field, and `walkProjectionByteOffset` refuses `ByteOffset` followed by `Field` while computing the offset, before any cell naming is attempted. Same blocker as `ReinterpretReadNestedFieldThroughIndex.cs`, reached from the write side. Not a regression: on the parent commit this program fails at its first statement instead.
            "ReinterpretReadNestedFieldThroughIndex.cs" // Reading a field of a nested struct directly through an inline-array index (`buf[k].I.P`), one step deeper than the `buf[k].Field` shape that now works. Not the cell resolver: `CliType.CellPathsExactlyCovering` descends to any depth and `TestCliTypeCellPaths` covers depth 3. The blocker is the projection chain — `readManagedByrefField`'s reinterpret-aware arms only fire when `ReinterpretAs` is last (or last-but-a-`ByteOffset`), so a trailing `Field` falls through to `readProjectedValue`, which cannot cross a reinterpret. Routing it to the byte-view reader is not enough either: the peeled chain is then `[ByteOffset k*sizeof(Elem); Field I; Field P]`, and `walkProjectionByteOffset` deliberately refuses `ByteOffset` followed by `Field` as an invariant violation. For the `Unsafe.Add(ref elem, k)` idiom the type anchor is in fact still live, but relaxing that guard changes an invariant used well beyond inline arrays, so it wants its own change. Not a regression: on the parent commit this program fails earlier still, at the write.
            ...
            "AssemblyGetNameSimpleName.cs" // `AssemblyNative_GetSimpleName` itself is implemented and unit-tested in `TestAssemblyNativeSimpleName.fs`; what is parked here is the only managed path a guest can reach it by. `Assembly.GetName()` fills an `AssemblyName` from six QCalls in a row (RuntimeAssembly.cs:112), and PawPrint answers only the first: it now gets past `GetSimpleName` and stops at the unimplemented `AssemblyNative_GetVersion` (`RuntimeAssembly::<GetVersion>g____PInvoke|71_0(QCallAssembly, *Int32, *Int32, *Int32, *Int32) -> void`) via `NativeCall.failUnimplemented` (Native/NativeCall.fs:689). `GetLocale`, `GetPublicKey`, `GetFlags` and `GetCodeBase` follow it. Un-park when those land.
