...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "AssemblyGetNameSimpleName.cs" // `AssemblyNative_GetSimpleName`, `_GetVersion`, `_GetLocale` and `_GetPublicKey` are implemented and unit-tested in `TestAssemblyNativeQCalls.fs`; what is parked here is the only managed path a guest can reach them by. `Assembly.GetName()` fills an `AssemblyName` from six QCalls in a row (RuntimeAssembly.cs:112), and PawPrint answers the first four: it now gets past `GetPublicKey` and stops at the unimplemented `AssemblyNative_GetFlags` (`RuntimeAssembly::GetFlags(QCallAssembly) -> AssemblyNameFlags`) via `NativeCall.failUnimplemented` (Native/NativeCall.fs:689). `GetCodeBase` follows it. Un-park when those land.
