...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "ActivatorCctorTypeInitializationTrace.cs" // The `TypeInitializationException` raised when a `.cctor` throws under `Activator.CreateInstance<T>()` reports a null `StackTrace`; real .NET names the failing type's constructor frame. Measured exit 3 (the null-trace branch) against 0. Specific to the *chained* wrap: `Activator.CreateInstance<T>()` sets `WrapExceptionInTargetInvocation` on the same frame that carries `WasInitialisingType`, so `ExceptionDispatching.applyFrameWraps` synthesises the TIE (with `StackTrace = []`) and re-wraps it in a `TargetInvocationException` on the same boundary, before a single frame is appended to it, and it surfaces as `InnerException` with neither frames nor `_stackTraceString`. Not general to synthesised TIEs — one reached by plain `newobj` propagates through frames and gets a trace, which is why this is a wrapper-frames gap and not a TIE gap. Same root cause as the frameless-token crash fixed in #870, and tracked as issue #865. The two-pass restructure deliberately preserved this: a wrap boundary ends one search and seeds the next at the caller's call site, which reproduces the old trace exactly, so what remains is the separate question of which frame a synthesised wrapper's own trace should *start* from. The assertion is a substring of the failing type's name rather than mere non-nullness, so an implementation that handed the TIE an empty or placeholder trace would still fail it (exit 4). `ActivatorCctorThrowsInnerStackTrace.cs` covers what does hold today — the read is reachable and does not throw — and passes.
