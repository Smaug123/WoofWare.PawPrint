...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "InterfaceSlotHiddenByDerivedMethod.cs" // Pre-existing, and reproducible with no variance involved: a derived class that only *hides* (`new`) a base class's implicit interface implementation wrongly takes over the interface slot. `IlMachineStateExecution.methodMatches` sets `allowImplicitInterfaceImplementation` whenever the call target's declaring type is an interface, which skips the guard rejecting non-virtual/`newslot` candidates, so the hidden method matches on name and signature and `findClassImplementation` — which walks the receiver before its base — picks it. Fixing this means teaching the class walk which type actually owns the slot, which changes ordinary interface dispatch and so wants its own change. The file's `CallExact` case is the root cause and fails identically on `main`.
            ...
            "ArrayClearReferenceElements.cs" // `Array.Clear` on an element type containing GC pointers branches to `SpanHelpers.ClearWithReferences(ref IntPtr, nuint)`, which — unlike its `ClearWithoutReferences` sibling covered by the passing ArrayClear.cs — is not `[Intrinsic]`, so it runs its managed IL: `Unsafe.Add(ref ip, n) = default` over an `Unsafe.As<byte, IntPtr>` view of the element data. That is a `stind.i` of a native-int zero through a byref covering a whole `ObjectRef` cell, and `IlMachineManagedByref.writeIndirectPrimitiveStore` sends it to the byte-view path, which fails with "refusing byte view over object reference". Fixing it means teaching the byref-write model that a pointer-width zero covering exactly one reference cell stores null — a change to a different subsystem from the intrinsic boundary, so it is deliberately left for its own change.
