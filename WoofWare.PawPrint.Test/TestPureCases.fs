...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "MakeGenericMethodConstraintSatisfied.cs" // The mirror of the passing MakeGenericMethodConstraintViolation.cs: arguments that *satisfy* a generic method's constraint must bind rather than throw, which is what stops an always-violating constraint check from passing the violation case. Binding succeeds and reflection now completes `RuntimeType.GetMethodBase` -- all four of its InternalCalls (`IsDynamicMethod`, `GetDeclaringType`/`GetMethodTable`, `IsConstructor`, `HasMethodInstantiation`) are implemented -- and stops afterwards on an unrelated gap in the pointer model: `Conv_I4` of an `Int64Source.WidenedNativeInt` carrying a `MethodRegistryHandle` (EvalStack.fs:358, "truncating pointer-shaped int64 to int32"). That is the shape of hashing a handle's `IntPtr`: on 64-bit `IntPtr.GetHashCode` is `long value = _value; value.GetHashCode()` (IntPtr.cs:90-94) and `Int64.GetHashCode` is `(int)l ^ (int)(l >> 32)`, so the pointer is widened and then truncated. `PointerHashSynthesis` already models the `MethodHandle` key; what is missing is synthesising bits for the widen-then-truncate path, which is its own change. Un-park when that lands.
            ...
            "AssemblyGetNameSimpleName.cs" // The six QCalls `AssemblyNative_GetSimpleName`, `_GetVersion`, `_GetLocale`, `_GetPublicKey`, `_GetFlags` and `_GetCodeBase` are implemented and unit-tested in `TestAssemblyNativeQCalls.fs`; what is parked here is the only managed path a guest can reach them by. `Assembly.GetName()` (RuntimeAssembly.cs:112) needs *nine* runtime primitives, not the six an earlier version of this comment claimed: after those six come `AssemblyNative_GetHashAlgorithm` and — because the metadata stream version of any modern image exceeds the `0x10000` guard — `ModuleHandle_GetMDStreamVersion` and `ModuleHandle_GetPEKind` on the manifest module (`RuntimeAssembly::GetManifestModule` itself is already implemented). PawPrint now gets past `GetCodeBase` and stops at the unimplemented `AssemblyNative_GetHashAlgorithm` (`RuntimeAssembly::GetHashAlgorithm(QCallAssembly) -> System.Configuration.Assemblies.AssemblyHashAlgorithm`) via `NativeCall.failUnimplemented` (Native/NativeCall.fs:702). Un-park when the remaining three land — and note that the managed code `GetName` calls may itself need primitives beyond that list, so the un-parking is what proves the path, not this enumeration.
