...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "TaskDelayWait.cs" // Found while decomposing issue #713. The blocker this was originally parked for (the `SystemNative_SchedGetCpu` PInvoke, issue #724) landed in #742, so `Task.Delay(1).Wait()` now gets as far as `TimerQueueTimer..ctor`, which picks its queue with `TimerQueue.Instances[(long) Thread.GetCurrentProcessorId() % Instances.Length]` and narrows that index with `conv.ovf.i` before the `ldelem.ref` (IL_0046). `Conv_ovf_i` is unimplemented (NullaryIlOp.fs:2368), filed as issue #753; the adjacent `Conv_ovf_u` case is implemented and shows the shape the fix takes.
        ]
        |> Set.ofList
