...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "ValueTypeHashCodeNullableField.cs" // A `Nullable<T>`-typed first field, which `ValueType_GetHashCodeStrategy` reports as `ValueTypeOverride` (a `Nullable<T>` is not bit-comparable and does override `GetHashCode`), so the guest boxes the field through the MethodTable we hand back. `RuntimeHelpers.Box` routes a nullable MethodTable to `CastHelpers.Box_Nullable` rather than `CastHelpers.Box`, and measured on top of the strategy handler the guest stops there: `System.Runtime.CompilerServices.MethodTable::InstantiationArg0() -> MethodTable*`, an unimplemented InternalCall that `Box_Nullable` uses to recover `T` so it can allocate a box of `T` rather than of `Nullable<T>`. That is a separate primitive from anything this QCall needs; the strategy itself is already right, and the file pins what the box must do once it exists — `Nullable<int>.GetHashCode` returns 0 both for "no value" and for the value 0, so checks 5 and 6 fail for any implementation that hashes the field's bytes instead of boxing it. Un-park when `MethodTable::InstantiationArg0` lands. Verified to exit 0 on real .NET.
            "ValueTypeHashCodeNativeIntField.cs" // A native-int field does not stop a struct being bit-comparable — CoreCLR's field loop rejects only `R4`/`R8` and non-bit-comparable nested value types (comutilnative.cpp:1637-1657) — so this struct takes `ValueType.GetHashCode`'s whole-image fast path and never reaches the strategy QCall at all. That is what `fieldAllowsFastCompare` was corrected to say, and this file is its observer: the hash must track *every* field, where a struct wrongly held non-bit-comparable would commit to the first one alone. It cannot pass yet, and not for any reason to do with the predicate: a native int is eight bytes and struct alignment rounds anything holding one plus a second field up to sixteen, at which point `HashCode.AddBytes` leaves its four-bytes-at-a-time path for the sixteen-byte block loop (HashCode.cs:359) and needs `Unsafe.Subtract(ref T, IntPtr)`, of which PawPrint implements only the `(ref T, int32)` overload. Measured: exactly that TODO, and `struct { long; long }` — no native int anywhere — fails identically today, so the gap is orthogonal. Un-park when that overload lands. Verified to exit 0 on real .NET.
