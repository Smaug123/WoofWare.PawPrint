...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "Threads.cs" // past the `Unsafe.As<TaskAwaiter<T>, TaskAwaiter>` nested-wrapper reinterpret (now handled by the Phase B transparent-wrapper classifier; see `UnsafeAsNestedSingleObjectWrapper.cs`); now blocks downstream at the unimplemented InternalCall `System.Runtime.CompilerServices.RuntimeHelpers::TryEnsureSufficientExecutionStack() -> bool`, which CoreLib uses on async resume paths
