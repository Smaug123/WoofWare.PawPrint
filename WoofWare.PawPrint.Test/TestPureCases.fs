...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "Threads.cs" // past `ThreadNative_YieldThread` on the spin path (now wired via `WhatWeDid.VoluntaryYield`); fails on Linux at the unimplemented PInvoke `EventSource_GetClrConfig` (XplatEventLogger reads the runtime tracing config at startup on non-Windows hosts)
