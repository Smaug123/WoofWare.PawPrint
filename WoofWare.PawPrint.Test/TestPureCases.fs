...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "VirtualDispatchGenericDefinitionSlots.cs" // The dispatch counterpart of `ReflectionVirtualSlotsGenericDefinitionLayout.cs`: which method answers a `callvirt` is decided by slot, and slots are laid out on the generic definition. `tryResolveByOverrideChain` -- which fixes the same shapes when no generic definition is involved, as `VirtualDispatchNewSlotShadow.cs` and `CovariantReturnDispatchMultiHop.cs` now pin -- declines this hierarchy by design, because its override relation compares signatures under an empty substitution context and at `T = string` that cannot tell `A3<T>.M(T)` from `B3<T>.M(string)`; a generic signature reaching that comparison fails loudly rather than mismatching quietly, and `VirtualDispatchGenericChainOverrides.cs` pins that the exclusion is what keeps these calls off it. Declining leaves the pre-existing `signatureMatchesTarget` path in charge, which concretises at each declaring type's arguments and so matches `C3<T>.M(T)` against a call site spelled `B3<string>::M(string)`, answering it where B3's slot still holds `B3.M`. Measured PawPrint, unchanged by that fix: 3 where .NET gives 2 (check 2), 13 where .NET gives 12 (check 4), and 23 where .NET gives 21 (check 5). Checks 1, 3 and 6 -- the calls through the slots those overrides really did take -- pass, so a failure is attributable to the choice of slot rather than to override lookup as a whole. Un-park when the override relation can read each ancestor's signature in its own type variables, which is the definition-level base walk in `VirtualSlotLayout`. That module now compiles before dispatch, so what remains is for dispatch to read its table rather than reconstruct the slot from declarations. Verified to exit 0 on real .NET.
