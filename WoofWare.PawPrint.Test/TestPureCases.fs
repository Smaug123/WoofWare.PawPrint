...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "AssemblyFullNameDisplayName.cs" // `Assembly.FullName` reaches the QCall `AssemblyNative_GetFullName` (`System.Private.CoreLib System.Reflection.RuntimeAssembly::GetFullName(QCallAssembly, StringHandleOnStack) -> void`), which is unimplemented, so this stops in `NativeCall.failUnimplemented` (Native/NativeCall.fs). Split out of `AssemblyGetNameSimpleName.cs` when that file un-parked: `Assembly.GetName()` and `Assembly.FullName` look adjacent but share no runtime primitive — `GetName` builds an `AssemblyName` from nine per-column QCalls, while `FullName` asks the runtime to format the whole display name in one. So this is a tenth primitive rather than a tenth step of the same chain.
