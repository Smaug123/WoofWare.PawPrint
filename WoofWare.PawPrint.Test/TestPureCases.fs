...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "MarshalOffsetOfBoolCharFields.cs" // `Marshal.OffsetOf` on a struct holding a `bool` or `char` field, and on `Nullable<int>`, whose `hasValue` is a `bool`. `MarshalNative_OffsetOf` answers from `CliValueType.TryComputeMarshalLayout`, the same walk `Marshal.SizeOf` uses, and that walk does not yet size either field: a `bool` marshals as a 4-byte BOOL and a `char` at its declaring type's `CharSet` width. Measured: "TODO MarshalNative_OffsetOf: unimplemented marshalling case for Program+WithBool ...: field B: System.Boolean marshals as a 4-byte BOOL by default, not a 1-byte CLI bool". Un-park when `CliValueType.TryFieldMarshalSize` sizes both, which will make `Marshal.SizeOf` of the same structs work too. Verified to exit 0 on real .NET.
