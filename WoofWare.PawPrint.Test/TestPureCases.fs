...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "ArrayClearPackedReferenceStruct.cs" // CoreCLR ignores `Pack = 1` for a value type containing GC pointers — references must stay pointer-aligned — so `struct { object O; byte B; }` is a 16-byte element there and `Array.Clear` walks it as two pointer slots. `CliValueType.ComputeConcreteFields` does not apply that rule, so PawPrint computes a 9-byte element and the slot count derived from it no longer lines up with the fields; the clear then asks to zero a range cutting across the reference and fails loudly ("a reference or pointer cannot be partially cleared"). Loud rather than silently leaving a field set, which is the right failure mode, but still a divergence. The fix belongs in struct layout — round a GC-containing value type up to pointer alignment — not in the clearing path, so it is left for its own change.
