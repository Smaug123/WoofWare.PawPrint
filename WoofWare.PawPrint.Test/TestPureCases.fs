...
module TestPureCases =
    ...
    let unimplemented =
        [
            "UnsafeAccessorBindsBeforeCctor.cs" // CoreCLR resolves an accessor's target while it compiles the stub, which is before the method's prologue, so a binding failure escapes without the accessor's own declaring type having been initialised. PawPrint services a frame's `PendingTypeInit` before it reaches the body, so the initialiser runs first and a guest watching a third type's counter sees one where real .NET shows zero (the counter has to live on a third type: reading one on the accessor's own type is itself the use that initialises it). Recorded in `docs/divergences.md`, "An `[UnsafeAccessor]` that fails to bind runs its own type's initialiser first". Un-park when the dispatch loop can let a body kind run before its own prologue. Verified to exit 0 on real .NET.
            ...
            "UnsafeAccessorArrayConstructor.cs" // An array is a legal `[UnsafeAccessor]` target -- modern CoreCLR gives arrays MethodTables rather than TypeDescs, so `ValidateTargetType` lets them through and the constructor binds, for `int[,]` and `int[]` alike. Only the constructor: an accessor naming `Get` reports `'System.Int32[,].Get'` missing, which this file also pins and which PawPrint already answers. The constructor is refused rather than answered wrongly: "TODO: [UnsafeAccessor] .TestUnsafeAccessorArrayConstructor::NewMultiDim names an array's constructor". `executeNewobj` reaches a multi-dimensional array's constructor only through the `MemberReference`-over-`TypeSpec` token an ordinary call site carries (`executeMultiDimArrayNewobj` decodes rank and element type from it), and reaches a single-dimensional array's not at all, since C# emits `newarr`. Un-park when those two allocations can be driven from a resolved element type and rank rather than from a token. Verified to exit 0 on real .NET.
            "UnsafeAccessorTypeByName.cs" // `[UnsafeAccessorType("...")]` names a parameter's or the return's type by string rather than in the signature, which is how CoreLib's own accessors reach `BinaryFormatter`, `GenericPrincipal` and `MetadataReader`. `MethodInfo.read` detects the attribute (`hasTypeNameOverrides`), and dispatch refuses on it rather than resolving the member against the signature's `System.Object`: "TODO: [UnsafeAccessor] .TestUnsafeAccessorTypeByName::NewHidden names at least one of its types with [UnsafeAccessorType]". Un-park when the attribute's blob is parsed and its type name resolved -- note that the name may be assembly-qualified, and that an unqualified one means the accessor's own assembly, which is the form this file uses. Verified to exit 0 on real .NET.
