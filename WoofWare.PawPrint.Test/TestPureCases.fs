...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "MakeGenericMethodConstraintSatisfied.cs" // The mirror of the passing MakeGenericMethodConstraintViolation.cs: arguments that *satisfy* a generic method's constraint must bind rather than throw, which is what stops an always-violating constraint check from passing the violation case. Binding does succeed, but reflection then continues into `RuntimeType.GetMethodBase`, and stops at the unimplemented `RuntimeMethodHandle.IsConstructor` (`System.RuntimeMethodHandle::IsConstructor(RuntimeMethodHandleInternal) -> bool`, runtimehandles.cpp:2135) reached at RuntimeType.CoreCLR.cs:1934; `HasMethodInstantiation` (runtimehandles.cpp:1722) stands behind it on the method branch. `GetMethodBase` is a run of consecutive InternalCalls (`IsDynamicMethod`, `GetDeclaringType`/`GetMethodTable`, `IsConstructor`, `HasMethodInstantiation`), so no guest source can pin any one of them alone: each is pinned by `TestMethodHandleRegistry.fs` driving the InternalCall directly, with their pure cores in `TestNativeRuntimeMethodHandle.fs`. Un-park when `IsConstructor` and `HasMethodInstantiation` land.
