...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "InlineArrayValueTypeElementWithReference.cs" // An `[InlineArray(N)]` whose element is a struct *containing* a reference, rather than a bare reference or a byte-addressable primitive. The N-slot layout is right for this shape (the `mixedRef` row of `TestInlineArrayLayout`'s differential sweep agrees with the real runtime on every size), and `TryFieldExactlyCovering` does find slot `k` exactly; what stops it is `isLayoutCompatibleForElision` (IlMachineManagedByref.fs), which is object-reference identity only, so the elision declines and the bytewise fallback then fails in `reinterpretStorageBytes` because a value type containing references has no byte rendering. Not an inline-array gap and not a regression: verified against `origin/main`, where element *0* of the same shape diverges identically with the same message. Un-parking it needs the elision predicate widened to accept two cells of the same declared value type, which changes the `Unsafe.As`-wrapper classifier for every caller and so wants its own change.
        ]
        |> Set.ofList
