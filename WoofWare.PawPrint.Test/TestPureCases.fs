...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "ReflectionInvokeConstructorOnInstanceManyArguments.cs" // `ConstructorInfo.Invoke(instance, args)` with more than four arguments, the only route to the five-argument `MethodBaseInvoker.InvokeConstructorWithoutAlloc` (MethodBaseInvoker.Constructor.cs:15). That overload's `shouldCopyBack` is a bare `stackalloc bool[argCount]` with no `NativeMemory.Clear`, unlike its `InvokeWithManyArgs` sibling, which clears a block covering all three regions (MethodBaseInvoker.cs:238-243) and so passes. CoreLib is compiled `[module: SkipLocalsInit]`, so `localloc` really does leave that block uninitialised on CoreCLR too, and `CheckArguments` writes `shouldCopyBack[i]` only on the conversion paths (MethodBaseInvoker.cs:354-386) — an argument already of the signature's type leaves its slot untouched. `MethodBaseInvoker.CopyBack` then *branches* on the slot it never wrote: real .NET reads the garbage and is saved only because the copy-back is a no-op either way (`copyOfParameters[i]` is the argument it was handed). PawPrint reads the slot as an undefined value, and the run ends at the branch rather than inventing one; measured: `RunOutcome.UndefinedValueObserved`, an undefined `UInt8` from byte 0 of the `localloc` block, observed as the operand of `brfalse.s` at IL offset 0x000d of `MethodBaseInvoker.CopyBack`. The guest's result does not depend on that byte, but its control flow does, and PawPrint refuses to choose an arm for it; un-parking needs CoreLib to clear the block, or a model that can see the two arms are equivalent. Verified to exit 0 on real .NET.
            ...
            "DelegateBindOpenGenericDefinitionDefaultMethod.cs" // `CreateDelegate` over a default interface method of an open variant interface definition, closed over a receiver implementing it by variance through `IChild : IContra<object>`, which overrides the default. The binding virtualises onto the receiver, and real .NET resolves the slot for the typical instantiation to the definition's own default body (11), although the closed `IContra<object>` dispatches to the override (22). PawPrint resolves through the closed instantiation, so it refuses any virtualisation that lands on a default body rather than bind the override: "TODO: Delegate_BindToMethodInfo must virtualise M of the open generic definition ... which implements it with a default interface method" (measured). Un-park when CoreCLR's default-method search for a typical instantiation is modelled. Verified to exit 0 on real .NET.
