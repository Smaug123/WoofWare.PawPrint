...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "MakeGenericMethodConstraintSatisfied.cs" // The mirror of the passing MakeGenericMethodConstraintViolation.cs: arguments that *satisfy* a generic method's constraint must bind rather than throw, which is what stops an always-violating constraint check from passing the violation case. Binding does succeed, but reflection then continues into `RuntimeType.GetMethodBase`, and stops at the unimplemented `RuntimeMethodHandle.HasMethodInstantiation` (`System.RuntimeMethodHandle::HasMethodInstantiation(RuntimeMethodHandleInternal) -> bool`, runtimehandles.cpp:1722), which `GetMethodBase` reaches on the method branch after `IsConstructor` returns false. `GetMethodBase` is a run of consecutive InternalCalls (`IsDynamicMethod`, `GetDeclaringType`/`GetMethodTable`, `IsConstructor`, `HasMethodInstantiation`), so no guest source can pin any one of them alone: each is pinned by `TestMethodHandleRegistry.fs` driving the InternalCall directly, with their pure cores in `TestNativeRuntimeMethodHandle.fs`. Un-park when `HasMethodInstantiation` lands.
