...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "AssemblyGetNameSimpleName.cs" // The seven QCalls `AssemblyNative_GetSimpleName`, `_GetVersion`, `_GetLocale`, `_GetPublicKey`, `_GetFlags`, `_GetCodeBase` and `_GetHashAlgorithm` are implemented and unit-tested in `TestAssemblyNativeQCalls.fs`; what is parked here is the only managed path a guest can reach them by. `Assembly.GetName()` (RuntimeAssembly.cs:112) needs nine runtime primitives: after those seven come — because the metadata stream version of any modern image exceeds the `0x10000` guard — `ModuleHandle_GetMDStreamVersion` and `ModuleHandle_GetPEKind` on the manifest module (`RuntimeAssembly::GetManifestModule` itself is already implemented). Those two are the first in this chain keyed by a `QCallModule` rather than a `QCallAssembly`. PawPrint now gets past `GetHashAlgorithm` and stops at the unimplemented `ModuleHandle_GetMDStreamVersion` (`System.ModuleHandle::GetMDStreamVersion(QCallModule) -> System.Int32`) via `NativeCall.failUnimplemented` (Native/NativeCall.fs:702). Un-park when the remaining two land — and note that the managed code `GetName` calls may itself need primitives beyond that list, so the un-parking is what proves the path, not this enumeration.
