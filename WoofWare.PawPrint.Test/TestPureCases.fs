...
module TestPureCases =
    ...
    let unimplemented =
        [
            "RawRootFieldPrefixPointerStructLeavesField.cs" // A byte view taken over a field of a struct holding a pointer, stepped out of that field: `Unsafe.Add(ref Unsafe.As<Inner, int>(ref p->I), 4)` reaching `Outer.Tail`, `Unsafe.Subtract` of the same byref reaching `Outer.Pad`, and `Unsafe.Subtract(ref p->I.B, 1)` reaching `A`. The byref itself is right — `[Field I; ReinterpretAs int; ByteOffset 16]` over the block's byte 0 — but the read path names a cell only at the chain's own structural prefix (`tryNameCellForByteAccess` over `Inner`), and when the access does not fit there `resolveCell` lifts to the enclosing struct and reads it *bytewise*, which a struct holding a pointer refuses. Measured: "refusing byte view over value type containing runtime pointers in single-cell byref <<<field 3::I ... of <byte 0 of <stack memory block #0> ...>> as System.Int32> + 16 bytes>". Not specific to raw roots: the same access through a local `Outer` stops at the same refusal with root `<variable 1 ...>`. `RawRootFieldPrefixByteOffset.cs` covers every one of these steps over a pointer-free struct. Un-park when naming a cell is tried at each level `resolveCell` lifts through, on the write path as well as the read. Verified to exit 0 on real .NET.
