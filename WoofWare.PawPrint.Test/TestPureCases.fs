...
module TestPureCases =
    ...
    let unimplemented =
        [
            "UnsafeAccessorAllowsRefStruct.cs" // An `[UnsafeAccessor]` whose own type parameter carries `allows ref struct` over a target parameter that does not. The absence of that anti-constraint is itself a constraint (`TypeVarTypeDesc::SatisfiesConstraints`, typedesc.cpp:1606), so real .NET refuses the byref-like instantiation with `VerificationException` while running the ordinary one. `acceptsWithoutChecking` detects the byref-like argument and refuses rather than running a target the real runtime would not bind: "TODO: [UnsafeAccessor] .TestUnsafeAccessorAllowsRefStruct::Id names a generic method whose type parameters do not accept the accessor's own type arguments outright". Un-park when PawPrint has a constraint-satisfaction check to put behind `VerifyDeclarationSatisfiesTargetConstraints`; that also unblocks every *constrained* target parameter, which is the wider half of the same gap. Verified to exit 0 on real .NET.
