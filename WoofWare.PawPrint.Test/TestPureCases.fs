...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "CustomAttributeTypeArgArrayType.cs" // A `System.Type`-valued attribute argument naming an array of a type of the decorated assembly, "Other[]". CoreLib's `TypeNameResolver` resolves the element type and then constructs the array through the `RuntimeTypeHandle_MakeSZArray` QCall, which is unimplemented: "Unimplemented native method (PInvokeImpl QCall!RuntimeTypeHandle_MakeSZArray)". `Type.GetType("Other[]")` takes the same path, so this is the resolver's array construction rather than anything in the attribute decoder; `RuntimeTypeHandle_MakeByRef` is the implemented sibling to model it on. Un-park when that QCall lands. Verified to exit 0 on real .NET.
            ...
            "AssemblyLoadCustomContext.cs" // `AssemblyLoadContext.LoadFromAssemblyName` on a context of the guest's own, which falls back to the default context and hands back the framework's System.Security.Claims. Constructing the context reaches `AssemblyNative_InitializeAssemblyLoadContext`, which is unimplemented; behind that, `AssemblyNative_InternalLoad` receives the context as its binder and refuses it, since PawPrint has only the one load context: "TODO: AssemblyNative_InternalLoad of 'System.Security.Claims' into a load context other than the default one". Un-park when the emulated runtime models a second load context, which is a question of what a custom context's `Load` override and `Resolving` event are allowed to observe. Verified to exit 0 on real .NET.
            "AssemblyLoadInvalidPublicKey.cs" // `Assembly.Load` of a display name whose `PublicKey=` is even-length hex (so the managed parser accepts it) but not a public key blob. `AssemblySpec::Init` runs `StrongNameIsValidPublicKey` over it and CoreCLR raises `SecurityException` "Invalid assembly public key."; `AssemblyNative_InternalLoad` decides the blob the same way and refuses rather than raising, since that exception is not one it raises: "TODO: AssemblyNative_InternalLoad of 'No.Such' names a 2-byte public key that StrongNameIsValidPublicKey rejects". Un-park when `SecurityException` joins `BaseClassTypes` and the handler raises it. Verified to exit 0 on real .NET.
