...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "NullDereferenceTest.cs" // past XOR on NativeInt(OpaqueHashBits) × NativeInt(TypeHandlePtr); now blocked reading through a `ReinterpretAs` projection (`IlMachineManagedByref.readProjectedValue`) from a `Numeric (Int32 0)` value reinterpreted as `.CastCacheEntry` — needs a bytewise read of the CastCacheEntry table cell
