...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "ReinterpretStindByteIntoReferenceStorage.cs" // `stind.i1` through `ref byte q = ref buffer[k].Tag` into an `[InlineArray]` slot whose element holds a reference; `stfld` to the same address works. `stind.i1` hands `writeIndirectPrimitiveStore` a signed `Int8`, the storage has no byte image, and the bytes writer's cell naming (`tryNameCellForByrefAccess`, strict `isCellIdentityCompatible`) refuses an `Int8` value for the `UInt8` cell, so the write falls to `resolveCell`. Measured: "refusing byte view over value type containing object references in single-cell byref <field ...::Tag ...>", from `writeManagedByrefBytesOrTypedCellCore`. Slot 0 fails identically, and so does the deeper `ref buffer[k].M.I.Q`. Relaxing the predicate is not the fix: installing the `Int8` verbatim restamps the cell, and the next `ldfld Tag` then fails to name it (measured). Un-park when a same-width primitive store into a named cell is converted to the cell's own kind. Verified to exit 0 on real .NET.
            ...
            "ReflectionOpenGenericDefinitionSharedParent.cs" // `typeof(D<>).GetMethods()` where the definition's base type mentions its own parameter (`SharedDerived<T> : SharedBase<T>`). The layout question is the one the active `ReflectionOpenGenericDefinitionMethods.cs` asks and PawPrint answers. Naming the parent is no longer the gap: `resolveBaseRuntimeTypeHandleTarget` substitutes the definition's formals into its extends clause, and `EnumQueriesOpenGenericSharedParent.cs` is active on exactly that. What fails is walking *into* the parent, the open construction `SharedBase<T of SharedDerived>`: `RuntimeType.PopulateMethods` asks it for `GetNumVirtuals`, and `VirtualSlotLayout.numVirtuals` refuses an `OpenConstructed` target. Measured: "TODO: open constructed types are not handled at VirtualSlotLayout.fs:<line>; got open constructed PawPrintTestAssembly/...[generic parameter #0 of ...]". Probed one step further by answering that from the definition, as `numVirtuals` already does for a bare definition: `GetIntroducedMethods` refuses the same shape next (`VirtualSlotLayout.introducedMethodsOf`), and the methods it would list need `MethodHandleRegistry` to mint handles whose declaring type is an open construction -- the registry limit `ReflectionBaseDefinitionOpenGenericBase.cs` is parked on too. The file is not satisfiable by approximating the parent as the *definition* `SharedBase<>`: checks 5 to 8 pin that the parent is the open construction, carrying the deriving definition's own type variable as its argument. Verified to exit 0 on real .NET.
            ...
            "ReflectionBaseDefinitionOpenGenericBase.cs" // `GetBaseDefinition` on a method of an open definition whose base definition is declared by a *generic* base. `GetBaseDefinition` asks the definition for its `BaseType` first, and that is now answered: the open construction `GenericBase<T of OpenDerived>`, from `resolveBaseRuntimeTypeHandleTarget`. Measured next: `RuntimeTypeHandle.GetNumVirtuals` on that parent, where `VirtualSlotLayout.numVirtuals` refuses an `OpenConstructed` target ("TODO: open constructed types are not handled at VirtualSlotLayout.fs:<line>") -- the same refusal `ReflectionOpenGenericDefinitionSharedParent.cs` reaches. Behind that, `RuntimeTypeHandle_GetMethodAt` refuses the same shape, because the answer's declaring type is the open construction and `MethodHandleRegistry` mints handles only over closed or definition declaring types -- the registry limit `MakeGenericMethodOpenArgument.cs` is parked on too. Un-park when slot queries answer for an open construction and a method handle can carry one as its declaring type. Verified to exit 0 on real .NET.
