...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "ThreadCultureSetBeforeStart.cs" // `Thread.CurrentCulture` set on a thread that has not started goes through `SetCultureOnUnstartedThread`, which reads `Thread.ThreadState` to check the thread is still unstarted; that is the `ThreadNative_GetThreadState` QCall, which PawPrint does not implement. The start path itself is ready for it: the worker runs CoreLib's `StartHelper.Run`, whose `InitializeCulture` installs the parked culture. Un-park when `ThreadNative_GetThreadState` is implemented. Verified to exit 0 on real .NET.
        ]
        |> Set.ofList
