...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "ActivatorCctorTypeInitializationTrace.cs" // The `TypeInitializationException` raised when a `.cctor` throws under `Activator.CreateInstance<T>()` does not name the failing type's constructor, which is real .NET's first frame for it. Measured exit 4 (trace present, wrong contents) against 0. It used to be exit 3, a null trace, because the chained wrap synthesised the TIE and re-wrapped it in a `TargetInvocationException` on the same boundary without appending a frame to it; every synthesised wrapper is now seeded with the frame that raises it, so the remaining gap is only *which* frame that is. Real .NET's trace is `Boom..ctor` then `RuntimeType.CreateInstanceOfT`, because CoreCLR triggers the initialisation from the constructor's own prologue, with that frame already established. PawPrint runs `loadClass` before pushing the callee on every call path, so the constructor frame does not exist when the TIE is born and the wrap seeds at the call site instead — the same missing-triggering-frame cause as the parked `CctorFailureTraceNamesTargetMethod.cs`, and the two will un-park together. Tracked as issue #865; same family as the frameless-token crash fixed in #870. The assertion is a substring of the failing type's name rather than mere non-nullness, so seeding alone does not satisfy it. `ActivatorCctorThrowsInnerStackTrace.cs` covers what does hold — both wrappers carry a readable, non-empty trace — and passes.
