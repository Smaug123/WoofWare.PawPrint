...
"""
    ...
    [<Test>]
    let ``Explicit MethodImpl on contravariant interface dispatches through more-specific instantiation`` () =
        // Regression for IlMachineStateExecution.findMatchingMethodImplBodies: when a class
...
"""
    ...
    [<Test>]
    let ``Variant MethodImpl matching disambiguates overloaded interface slots`` () =
...
"""
        ...
        runPawPrintSource
            "VariantOverloadedMethodImpl.cs"
            source
            (MockEnv.make ())
            Map.empty
            (fun _image pawPrintResult ->
