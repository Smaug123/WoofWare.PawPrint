...
module TestPureCases =
    ...
    let unimplemented =
        [
            "ArrayCloneThroughICloneable.cs" // `Array.Clone` itself is implemented (see ArrayClone.cs), but `IlMachineStateExecution.callMethod` performs its `[Intrinsic]` dispatch *before* virtual/interface resolution. `callvirt ICloneable::Clone()` therefore resolves to `Array.Clone` only after the intrinsic check has already passed, so PawPrint executes Array.Clone's IL body and reaches the unimplemented `Object.MemberwiseClone` boundary. Fix is to re-check intrinsic-ness against the post-resolution method.
