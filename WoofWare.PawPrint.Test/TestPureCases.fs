...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "ReflectionInvokeMethodManyArguments.cs" // `MethodBase.Invoke` on a target taking more than `MethodBaseInvoker.MaxStackAllocArgCount` (4) arguments, which routes through `InvokeWithManyArgs`. Up to four arguments now pass (`ReflectionInvokeMethodMultipleArguments.cs`); what blocks this is neither reflection nor the byref buffer but `System.Runtime.GCFrameRegistration::RegisterForGCReporting`, an unimplemented InternalCall that `InvokeWithManyArgs` calls to report its `stackalloc`d argument block to the GC. Observed by un-parking and running. Un-park when that InternalCall lands.
