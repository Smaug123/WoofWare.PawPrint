...
module TestPureCases =
    ...
    let unimplemented =
        [
            "NestedFinallyOnLeave.cs" // A `leave` that exits more than one enclosing protected region runs only the innermost `finally`, then jumps to the target; every outer `finally` on the way out is skipped (ECMA-335 III.3.55 requires all of them, innermost first). `UnaryConstIlOp.leave` gets the full list right via `Exceptions.findFinallyBlocksToRun` — sorted inner-to-outer — and then matches `finallyOffset :: _`, discarding the tail and pushing a single `FinallyHandler` continuation of `ResumeAfterFinally targetPc`. Whether C# emits a multi-region `leave` depends on where the branch target lands, so the file includes the one-region control that passes today alongside the loop-exit, `return` and `goto` shapes that fail. Handlers append to a shared ordered trace rather than bumping independent counters, so the file also pins ECMA-335's innermost-first *ordering*: a fix that ran every handler but folded the inner-to-outer list the wrong way round would satisfy per-handler counts, and is caught here instead (exit 42). Note the scenarios spell their loop exit `goto` rather than `break`: `Roslyn.compile` builds unoptimized, and Roslyn lowers `break` to a `leave` targeting a label still inside the outer `try` followed by a second `leave` out of it — one region each, which PawPrint already handles — folding them into a single multi-region `leave` only when optimized, which is how the Release-compiled CoreLib comes by the shape. Measured under the harness: exit 21. Fixing it means chaining the remaining handlers, which changes `leave` for every guest, so it wants its own change.
            "CancellationTokenRegistrationDisposeCrossThread.cs" // Guest-level witness of `NestedFinallyOnLeave.cs`, and the shape a user actually hits. `CancellationTokenSource.ExecuteCallbackHandlers` `break`s out of its dispatch loop with nothing following the loop inside the outer `try`, so the `leave` skips the outer `finally` that sets `_state = NotifyingCompleteState` and clears `Registrations.ExecutingCallbackId`; a later `CancellationTokenRegistration.Dispose()` from a thread other than the canceller then spins for ever in `Registrations.WaitForCallbackToComplete`. NOTE: this file *hangs* rather than failing, so it must not be un-parked before the `leave` fix lands — parking is safe because the `unimplemented` fixture only runs these against the real runtime.
            ...
            "DelegateToStaticMethodRunsCctor.cs" // Invoking a delegate whose target is a static method does not run that type's `.cctor`. `ldftn` is right to defer it — taking a function pointer is not a use of the type — but the call through the pointer is a use, and `AbstractMachine.dispatchDelegateInvoke` goes straight to `IlMachineStateExecution.callMethod` with no `loadClass`, so the initialiser and its side effects are skipped entirely. Found by Codex review of the `constrained. ldftn` change (#877), but *not* introduced by it: the file's first half uses a plain `ldftn` of an ordinary static method and diverges identically on `main` (real .NET returns 0, PawPrint 2), and the `constrained.` path merely reaches the same `dispatchDelegateInvoke`. The file therefore currently fails at code 2, before the constrained half is reached; both halves are checked once the shared fix lands. The direct-call controls pass, which is what makes this about delegate invocation rather than type initialisation generally. Un-parking means teaching `dispatchDelegateInvoke` to initialise the target's declaring type — which needs the suspend/retry protocol every other `loadClass` caller has, and reaches every delegate in the interpreter, so it is its own change.
