...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "ArrayGetInterfacesPointerElement.cs" // `RuntimeTypeHandle.GetInterfaces` handles a pointer-element array correctly (it is an ordinary SZ array handle, and `PopulateInterfaces` declines to add the implicit generic five because `arrayType.IsPointer`), but the test can't reach that far: materialising the `RuntimeType` for the *element* `int*` needs `TypeHandle.AsTypeDesc` (`handle & ~2`), which `andNativeIntAddressBits` refuses because the result is the target's `TypeDesc*` — an identity `NativeIntSource` has no case for. Pre-existing and unrelated to interfaces; the sibling `RuntimeTypeHandleFromIntPtr.cs` below reaches the same blocker by the shortest route, and un-parking either needs the `TypeDescPtr` identity planned in docs/plans/2026-08-06-typehandle-tag-bits.md. (Before that refusal existed, the mask silently produced `Verbatim 0` and this surfaced one step later as `TODO: ldflda System.Runtime.CompilerServices.TypeDesc::_exposedClassObject through native pointer 0` at `UnaryMetadataFieldOps.fs:448`.)
            ...
            "TaskDelayWait.cs" // Found while decomposing issue #713. Two blockers this was parked for have now landed: the `SystemNative_SchedGetCpu` PInvoke (issue #724, in #742) and `Conv_ovf_i` (issue #753), which `TimerQueueTimer..ctor` needs to narrow `TimerQueue.Instances[(long) Thread.GetCurrentProcessorId() % Instances.Length]` before its `ldelem.ref`. `Task.Delay(1).Wait()` now gets past both and stops at the next primitive: the unimplemented `SystemNative_GetUInt64OSThreadId` PInvoke (`System.Private.CoreLib .Sys::GetUInt64OSThreadId() -> System.UInt64`), via `NativeCall.failUnimplemented` (Native/NativeCall.fs:683). It is reached from `System.Threading.Lock.ThreadId.InitializeForCurrentThread` (Lock.NonNativeAot.cs:52), whose body is `#if`-split per target: the `TARGET_OSX` branch calls `GetUInt64OSThreadId`, while a Linux CoreLib compiles the `TryGetUInt32OSThreadId` branch instead. PawPrint always interprets the *host's* CoreLib (every entry point selects it via `DotnetRuntime.SelectForDll`), so which of the two you hit depends on the machine: macOS dev boxes see the first, CI (ubuntu-latest) the second. Neither is implemented, so this is parked either way — but a fix must cover both entry points to un-park it everywhere. Both derive from one upstream quantity, `minipal_get_current_thread_id()` (`gettid` on Linux, `pthread_threadid_np` on macOS), so they want one emulated-kernel notion between them rather than two independent handlers. That is now the *only* remaining blocker: stubbing both entry points, on top of the `newobj` cctor-contention forwarding that landed with `NewobjCctorRace.cs`, runs this case to a clean exit. `conv.ovf.i` coverage now lives in the passing `ConvOvfI.cs` instead.
            "ComparerDefaultNullable.cs" // The `Nullable<T>` branch of `ComparerHelpers.CreateDefaultComparer` is the only one of the four that needs reflection PawPrint lacks: `int?` does not implement `IComparable<int?>`, so selection falls through to `type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>)` and stops at the unimplemented QCall `RuntimeTypeHandle_GetGenericTypeDefinition` (`System.RuntimeTypeHandle::GetGenericTypeDefinition(QCallTypeHandle, ObjectHandleOnStack) -> void`), via `NativeCall.failUnimplemented` (Native/NativeCall.fs:683). The other three branches — `GenericComparer<T>`, `EnumComparer<T>` and the `ObjectComparer<T>` fallback — short-circuit before that test and pass; they are covered by the sibling `ComparerDefault.cs` (the enum branch escapes because `IsGenericType` is false, so `&&` short-circuits before the QCall). The gap is in reflection, not in anything comparer-specific.
            ...
            "RuntimeTypeHandleFromIntPtr.cs" // `RuntimeTypeHandle.FromIntPtr` runs `GetRuntimeTypeFromHandle`, whose managed IL branches on `TypeHandle.IsTypeDesc` (`handle & 2`, answered correctly) and then calls `TypeHandle.AsTypeDesc` (`handle & ~2`) to reach `TypeDesc::_exposedClassObject`. That mask preserves the whole address and clears the tag, so its result is the target's `TypeDesc*` — a distinct identity `NativeIntSource` has no case for, and `andNativeIntAddressBits` now refuses it loudly instead of silently answering `Verbatim 0` (a null TypeDesc pointer, which then failed one step later at `TODO: ldflda ... through native pointer 0`). Un-parking this needs the `TypeDescPtr` identity plus its `_exposedClassObject` projection, planned in docs/plans/2026-08-06-typehandle-tag-bits.md; that is the same missing piece `ArrayGetInterfacesPointerElement.cs` above is blocked on, reached here by the shortest route. The MethodTable-shaped half of the file (`typeof(int)`) takes `AsMethodTable`, which is a plain cast with no mask, and is unaffected.
            "PathGetFullPathRelative.cs" // `Path.GetFullPath` on a *relative* path must resolve the process's current directory before it can collapse segments, which on Unix is `Interop.Sys.GetCwd()`. `SystemNative_GetCwd` has no handler registered in `Native/NativeDispatch.fs` at all, so this stops at `NativeCall.failUnimplemented`. The current directory is exactly the kind of host-visible value `EmulatedKernel` exists to hold as data, so the fix is a kernel-state field plus a handler, not a host `Directory.GetCurrentDirectory()` read. The rooted-path half of `GetFullPath`, which skips the lookup and exercises `PathInternal.RemoveRelativeSegments`, passes and is covered by the sibling `PathGetFullPathRooted.cs`.
            "InlineArrayOfReferenceType.cs" // Write side of the `[InlineArray(N)]`-over-a-reference-element shape. Indexing such a struct lowers to `Unsafe.As<TBuffer, TElement>(ref buffer)`, i.e. a byref whose projections are `[ReinterpretAs TElement]` with *no* trailing `Field`/`ByteOffset`, over storage whose single field spans the whole cell. `buffer[0] = "hello"` fails in `reinterpretStorageBytes` (IlMachineManagedByref.fs:2046) with "write through `ReinterpretAs` over byte-unaddressable storage (value type containing object references) is not modelled". This is *not* the array-write case #748 fixed: that one zero-fills through `writeArrayBytes`/`CliType.WithZeroedRangeIfChanged`, whereas this write carries an arbitrary new reference and is rooted at a local. The nearby `classifyTransparentWrapper` (IlMachineManagedByref.fs:1213) already treats "storage is a one-field offset-0 wrapper" as a sound elision, but only when reached via a trailing `[Field f]` into a reinterpret target that is *itself* such a wrapper; here the target (`System.String`) is the field's own type with no further projection, which it does not yet match. Extending that classifier to the no-projection case — via `transparentWrapperFastPath` (IlMachineManagedByref.fs:2132) on the write side — is the intended fix, and must keep rejecting a struct whose reference does *not* span the whole storage. Deliberately confined to a *write* at element 0, observed through a `First` property that reads `_item` as an ordinary field: element 1 would be the separate tail gap (`InlineArraySecondSlot.cs`) and reading back through the indexer would be the separate read gap (`InlineArrayOfReferenceTypeReadOnly.cs`), either of which would stop the write-side fix from un-parking this on its own.
            "InlineArrayOfReferenceTypeReadOnly.cs" // Read side of the same shape as `InlineArrayOfReferenceType.cs`, isolated so the two directions can be fixed and verified independently: reading element 0 of a default-initialised (all-null) buffer fails even though no write ever happens, with "refusing byte view over value type containing object references in single-cell byref". `readManagedByref`'s zero-offset reinterpret short-circuit (`elideAsObjectRefCell`, IlMachineManagedByref.fs:1150) only elides when the storage cell is *already* `CliType.ObjectRef` — the `Unsafe.As<object, TRef>` motif that `UnsafeAsObjectFieldAsRefType.cs` covers — so a `CliType.ValueType` storage cell declines and falls through to `readManagedByrefBytesAs` (IlMachineManagedByref.fs:1006), whose `resolveCell` (IlMachineManagedByref.fs:1072) reaches `byteAddressableCellSize`/`validateByteAddressableCell` (IlMachineManagedByref.fs:486/476) and is rejected.
