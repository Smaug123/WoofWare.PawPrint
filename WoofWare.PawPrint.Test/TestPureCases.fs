...
module TestPureCases =
    ...
    let unimplemented =
        [
            "MixedArrayByrefSubtraction.cs" // Subtracting two byrefs into one array where only one carries a byte view: "refusing to subtract array element pointer from incompatible pointer: <<element 0 of array> as System.Int32> vs <element 0 of array>". `subManagedPtrs` has both mixed-direction arms for stack-memory, native-memory and string roots (BinaryArithmetic.fs) and neither for array roots, so the pair reaches the catch-all refusal. The two operands here differ only in how each byref reached the native-int stack: `Unsafe.AsPointer` transports a plain array-element byref, while `fixed` goes through `conv.u`, which anchors a byte view. Measured on `main` before the byte-arithmetic change, so this is a pre-existing gap rather than a consequence of it; `docs/plans/2026-09-08-array-byref-byte-arithmetic.md` calls it defect (B). Un-park when those two arms exist. Verified to exit 0 on real .NET.
            "PointerArrayCellThroughByteView.cs" // Reading a cell of an `int*[]` back through a byte view of the array's data: "refusing byte view over runtime pointer in array <object> element 0". `MemoryMarshal.GetArrayDataReference(Array)` anchors a byte-stride cursor unconditionally (`anchorByteStrideOverArrayData`), and a whole-cell read back through it has to notice the cursor is on a cell boundary and hand over the cell. `readArrayBytesAs`'s shape-matching short-circuit does that for numeric and reference cells, but `sameCliConstructor` has no arm pairing a stored `RuntimePointer` with the `NativeInt` template an `nint`-shaped read presents, so the read falls through to a byte walk that a pointer cell has no image for. The write side already accepts that pairing (`destinationNeedsWholeCellStore`), and so does a bare `ldind.i` of the same cell, so the read is the odd one out. The two guests that already touch pointer arrays say the same thing in prose (`FixedArrayPointerArithmetic.cs`, `ArrayDataReferenceByteStride.cs`). Un-park when that pairing is accepted. Verified to exit 0 on real .NET.
            "ReflectionFieldGetValueGenericStatic.cs" // `FieldInfo.GetValue` on a static field of a *closed generic* type. The read itself is the `RuntimeFieldHandle_GetValue` QCall, which is implemented (`ReflectionFieldGetValue.cs`), but the guest never reaches it: `RuntimeType.PopulateRtFields` (RuntimeType.CoreCLR.cs:916) enumerates a closed generic type's fields and, for each *static* one, calls the InternalCall `RuntimeFieldHandle.GetStaticFieldForGenericType` to swap the canonical FieldDesc for the exact instantiation's (runtimehandles.cpp:2220). Measured: "Unimplemented native method (InternalCall): System.RuntimeFieldHandle::GetStaticFieldForGenericType(System.RuntimeFieldHandleInternal, *(System.Runtime.CompilerServices.MethodTable)) -> System.RuntimeFieldHandleInternal". Any `GetField` on such a type hits it, so the file's instance-field check is behind the same blocker. PawPrint's field-handle registry already keys a handle on the exact declaring instantiation, so the fixup is expected to be the identity on a handle whose declaring type is the MethodTable argument. Un-park when that InternalCall lands. Verified to exit 0 on real .NET.
            ...
            "UnsafeAccessorOnGenericType.cs" // An accessor declared *on a generic type*, which is the documented way to reach a generic type's members: the two types' variables agree positionally, and CoreCLR compares their signature blobs with no substitution on either side (unsafeaccessors.cpp:399/409), so `!0` matches `!0` by index alone. PawPrint's `signaturesEquivalent` identifies a variable by its owning definition as well as its index and refuses to compare two owners, which is what a definition-rooted vtable walk needs; the refusal is "TODO: [UnsafeAccessor] .Accessors`1::New is declared on a generic type". Un-park when `SubstitutionArgument` can express a variable identified by position alone -- a third case beside `Formal`, since weakening `Formal` would weaken the vtable layout that relies on it. `sourcesPure/UnsafeAccessorGenericTarget.cs` covers the shapes that do work today. Verified to exit 0 on real .NET.
            "UnsafeAccessorMethodOfGenericType.cs" // A *method* of a generic type named from an accessor on a non-generic type. The lookup binds it and CoreCLR's `VerifyDeclarationSatisfiesTargetConstraints` then refuses with `InvalidProgramException`, because the declaration supplies no class instantiation for the target's parameters; that is not one of the three exceptions accessor dispatch raises (`BadImageFormatException`, `MissingMethodException`, `MissingFieldException`), so the shape is refused instead: "TODO: [UnsafeAccessor] .TestUnsafeAccessorMethodOfGenericType::MethodOfGenericType names a method of the generic type ...". Un-park when `UnsafeAccessorRefusal` grows an `InvalidProgram` case; `InvalidProgramException` is already in `BaseClassTypes`. Verified to exit 0 on real .NET.
            "UnsafeAccessorAbstractConstructor.cs" // A `Constructor` accessor naming an abstract class's constructor. The lookup binds it and the synthesised `newobj` then refuses: CoreCLR raises `InvalidOperationException` (`Acc_CreateAbst`), which is not one of the three exceptions accessor dispatch raises, so the shape is refused instead: "TODO: [UnsafeAccessor] .TestUnsafeAccessorAbstractConstructor::NewAbstract names a constructor of the abstract class Abstract". Note the order this file's sibling `UnsafeAccessorRefusedTargets.cs` pins: the lookup answers first, so an abstract class with no matching constructor reports the missing constructor. Un-park when `UnsafeAccessorRefusal` grows an `InvalidOperation` case. Verified to exit 0 on real .NET.
            "UnsafeAccessorRefStructConstraintViolation.cs" // A generic target instantiated with a byref-like type for a parameter that does not say `allows ref struct`: the *absence* of that anti-constraint is itself a constraint (typedesc.cpp:1606), and CoreCLR raises `VerificationException` with `IDS_EE_METHOD_CONSTRAINTS_VIOLATION`. PawPrint decides the pair the same way and refuses rather than raising, since that exception is not one accessor dispatch raises and `BaseClassTypes` does not carry it: "TODO: [UnsafeAccessor] .TestUnsafeAccessorRefStructConstraintViolation::Id instantiates the generic method Id with the byref-like type System.Span`1<System.Int32> for a type parameter U that does not say `allows ref struct`". `sourcesPure/UnsafeAccessorAllowsRefStruct.cs` holds the instantiations that bind. Un-park when `VerificationException` joins `BaseClassTypes` and `UnsafeAccessorRefusal` grows a case for it. Verified to exit 0 on real .NET.
            "UnsafeAccessorStructVirtualIsAmbiguous.cs" // A value type's *virtual* method -- an `override`, or an implicitly implemented interface method, which is `virtual final newslot` in metadata -- is two candidates to CoreCLR, not one: it generates an unboxing stub beside every such method and `IntroducedMethodIterator` yields both, so the lookup raises `AmbiguousMatchException`. PawPrint detects the shape and refuses, since that exception is not one accessor dispatch raises and `BaseClassTypes` does not carry it: "TODO: [UnsafeAccessor] .TestUnsafeAccessorStructVirtualIsAmbiguous::Overridden names the virtual method ToString of a value type". Un-park when `AmbiguousMatchException` joins `BaseClassTypes` and `UnsafeAccessorRefusal` grows a case for it. Verified to exit 0 on real .NET.
            "UnsafeAccessorSharedGenericTarget.cs" // A `ref T` target position whose `T` is a reference type. A generic method over a reference type is compiled once, for `System.__Canon`, so CoreCLR resolves the target against that shared instantiation, which declares nothing: measured as `'System.__Canon.x'` missing however real the member is on the argument (a struct `T` is not shared and binds; `sourcesPure/UnsafeAccessorByrefGenericTarget.cs`). PawPrint has no shared instantiation and would bind the member on the exact type, so it refuses: "TODO: [UnsafeAccessor] .TestUnsafeAccessorSharedGenericTarget::Field reaches a reference type through `ref T`". This file also names an array directly as a field accessor's target, which is the array refusal `UnsafeAccessorArrayMemberLookup.cs` describes. Un-park when dispatch can answer a shared-instantiation lookup with the `MissingFieldException`/`MissingMethodException` CoreCLR reports, naming `System.__Canon`. Verified to exit 0 on real .NET.
            "UnsafeAccessorArrayMemberLookup.cs" // An array as an accessor's target type. Modern CoreCLR gives arrays MethodTables rather than TypeDescs, so `ValidateTargetType` lets them through; every member but the constructor is then reported missing under the array's reflection name (`'System.Int32[,].Get'`, `'System.Int32[]._numComponents'`). PawPrint refuses an array target for every kind, rather than answering the lookup for some kinds and not the constructor: "TODO: [UnsafeAccessor] .TestUnsafeAccessorArrayMemberLookup::RankTwoGet names an array type as its target". Un-park together with `UnsafeAccessorArrayConstructor.cs`, or alone by answering the non-constructor kinds with the missing-member exception under the reflection rendering of the array name. Verified to exit 0 on real .NET.
            "UnsafeAccessorArrayConstructor.cs" // An array's constructor as an accessor's target: legal on CoreCLR, and it binds for `int[,]` and `int[]` alike. PawPrint refuses an array target for every kind: "TODO: [UnsafeAccessor] .TestUnsafeAccessorArrayConstructor::NewMultiDim names an array type as its target". `executeNewobj` reaches a multi-dimensional array's constructor only through the `MemberReference`-over-`TypeSpec` token an ordinary call site carries (`executeMultiDimArrayNewobj` decodes rank and element type from it), and reaches a single-dimensional array's not at all, since C# emits `newarr`. Un-park when those two allocations can be driven from a resolved element type and rank rather than from a token. Verified to exit 0 on real .NET.
            ...
            "MarshalStructureToPtrDecimalField.cs" // A round trip of `{ int; decimal }` through `Marshal.StructureToPtr` and back through `Marshal.PtrToStructure`. The outbound half works: the struct gets a synthesised marshal stub, which copies the Decimal verbatim at native offset 8, and `MarshalStructureToPtrDecimalFieldBytes.cs` pins that image byte by byte. What stops it is the way back. `PtrToStructureHelper` calls the same stub with `MarshalOperation.Unmarshal` (Marshal.CoreCLR.cs:291), and the stub implements only `Marshal` and `Cleanup`; measured: "TODO struct-marshal stub: native-to-managed direction (MarshalOperation.Unmarshal, reached via Marshal.PtrToStructure) is not implemented". Nothing about that is Decimal-specific -- a `DateTime` field hits the same wall -- so un-park when the stub learns to unmarshal.
            ...
            "ReflectionBaseDefinitionOpenGenericBase.cs" // `GetBaseDefinition` on a method of an open definition whose base definition is declared by a *generic* base. Measured: "TODO: resolveBaseRuntimeTypeHandleTarget for open generic typedef ... base type ... references generic parameters" -- `GetBaseDefinition` asks the definition for its `BaseType` first, and a base that is an open construction is not answered. Behind that, `RuntimeTypeHandle_GetMethodAt` refuses the same shape, because the answer's declaring type is the open construction `GenericBase<T of OpenDerived>` and `MethodHandleRegistry` mints handles only over closed or definition declaring types -- the registry limit `MakeGenericMethodOpenArgument.cs` is parked on too. Un-park when a definition's base can be an open construction and a method handle can carry one as its declaring type. Verified to exit 0 on real .NET.
            ...
            "ReflectionInvokeOutNullSlot.cs" // `MethodBase.Invoke` with a null `object[]` slot for an `out` value-type parameter. `RuntimeType.TryChangeType` (RuntimeType.cs:946) fills the slot with `default(T)` through `RuntimeType.AllocateValueType(elementType, null)`, which is `RuntimeHelpers.GetUninitializedObject`, and that stops at the unimplemented QCall `ReflectionSerialization_GetCreateUninitializedObjectInfo` (`CreateUninitializedCache.GetCreateUninitializedInfo`) before any byref is formed (measured). The byref pass-through itself is covered by `ReflectionInvokeByRefParameter.cs`, which passes a boxed default instead; ASP.NET's route through the LINQ interpreter does the same, so nothing on the ladder waits on this. Un-park when that QCall lands. Verified to exit 0 on real .NET.
            ...
            "StackTraceGenericDeclaringFrame.cs" // A current-thread capture taken from inside a method of a *generic* type, which `StackTraceCurrentThreadFrames.cs` (now passing) cannot reach: every frame that one captures is declared on a non-generic type. The difference decides whether the capture needs a QCall at all. CoreCLR strips only the *method* instantiation from each captured frame (debugdebugger.cpp:449-452), so a frame on `Holder<int>.Capture` still carries its class instantiation; that answers false to `RuntimeMethodHandle.IsTypicalMethodDefinition`, and `GetTypicalMethodDefinition` (RuntimeHandles.cs:1291-1300) then falls through to the QCall. Measured: "Unimplemented native method (PInvokeImpl QCall!RuntimeMethodHandle_GetTypicalMethodDefinition): System.RuntimeMethodHandle::GetTypicalMethodDefinition(System.RuntimeMethodHandleInternal, System.Runtime.CompilerServices.ObjectHandleOnStack) -> void". That QCall must rebind the handle onto `RuntimeTypeHandleTarget.OpenGenericTypeDefinition` and hand back a fresh `RuntimeMethodInfoStub` for it (`MethodDesc::LoadTypicalMethodDefinition` then `AllocateStubMethodInfo`, runtimehandles.cpp:1806-1826), and everything downstream then has to answer reflection questions about a method of an open definition -- the wall `NativeRuntimeMethodHandle.requireClosedDeclaringType` documents. Un-park when that QCall lands. What the file pins beyond un-parking: that the reported frame is the *typical* definition, so its declaring type is `Holder<T>` and answers `IsGenericTypeDefinition`, not `Holder<int>` -- which is what would go red if the QCall were implemented as a no-op returning the caller's own handle. Verified to exit 0 on real .NET.
