...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "TaskRunThrowSetsFaulted.cs" // Found while decomposing issue #713. Two blockers down: the `ldsflda` through a `MemberReference` token (#723) landed in #740, and the `ExceptionNative_GetFrozenStackTrace` QCall (#754) landed with `sourcesPure/ExceptionDispatchInfoCapture.cs`. Task's fault path now stops at the next primitive, the unimplemented InternalCall `System.Private.CoreLib System.Exception::IsImmutableAgileException(System.Exception) -> System.Boolean`. That is reached from `Exception.RestoreDispatchState` (Exception.CoreCLR.cs:125-148), because the thread-pool dispatch loop rethrows the captured fault through `ExceptionDispatchInfo.Throw()` — so, like the QCall before it, this still reproduces from the minimal `t.IsFaulted` spin-check alone and remains inside Task's own fault-recording rather than anything downstream that user code touches. Measured: stubbing `IsImmutableAgileException` to `false` and `Exception::PrepareForForeignExceptionRaise` (the other InternalCall `RestoreDispatchState` needs) to a no-op takes this file to a passing exit 0, so those two are the whole remaining runway. Un-park when both land; note that `ex.StackTrace` will then show only the post-rethrow frames, because `RestoreDispatchState` nulls `_stackTraceString` and PawPrint does not yet consume the foreign-raise flag to re-prefix the restored trace — this file asserts only on `IsFaulted`/`InnerException` type, so it is insensitive to that, but a test asserting trace *content* would not be.
