...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "AssemblyGetNameSimpleName.cs" // The eight QCalls `AssemblyNative_GetSimpleName`, `_GetVersion`, `_GetLocale`, `_GetPublicKey`, `_GetFlags`, `_GetCodeBase`, `_GetHashAlgorithm` and `ModuleHandle_GetMDStreamVersion` are implemented and unit-tested in `TestAssemblyNativeQCalls.fs`; what is parked here is the only managed path a guest can reach them by. `Assembly.GetName()` (RuntimeAssembly.cs:112) needs nine runtime primitives, the last being `ModuleHandle_GetPEKind` on the manifest module (`RuntimeAssembly::GetManifestModule` itself is already implemented). PawPrint now gets past `GetMDStreamVersion` and stops at the unimplemented `ModuleHandle_GetPEKind` (`System.ModuleHandle::GetPEKind(QCallModule, *(System.Int32), *(System.Int32)) -> void`) via `NativeCall.failUnimplemented` (Native/NativeCall.fs:702). That it is reached at all is itself the confirmation that `GetMDStreamVersion` answers above the `0x10000` guard the call sits behind. Un-park when that last one lands — and note that the managed code `GetName` calls may itself need primitives beyond this list, so the un-parking is what proves the path, not this enumeration.
