...
module TestPureCases =
    ...
    let unimplemented =
        [
            ...
            "NullDereferenceTest.cs" // past NativeIntSource.OpaqueHashBits round-trip on the `RotateLeft(nuint, int)` `(nuint)` cast; now blocked because the cast cache hash `hash = RotateLeft(source, 32) ^ target` reaches `Xor` on `NativeInt(OpaqueHashBits)` and `NativeInt(TypeHandlePtr)` — pointer-shape bit-mixing in the NativeInt domain is not yet wired through PointerHashSynthesis.materialiseHashBits.
