...
module TestFunctionPointerConcretization =
    ...
    let ``Concretizing TypeDefn.FunctionPointer yields ConcreteTypeHandle.FunctionPointer`` () : unit =
        ...
        | ConcreteTypeHandle.FunctionPointer concreteSig ->
            ...
            | ConcreteFunctionPointerReturnType.Returns _ -> ()
            | ConcreteFunctionPointerReturnType.Void -> Assert.Fail "Expected Returns, got Void"
    ...
    let ``Function pointer parameters are concretized`` () : unit =
        ...
        match handle with
        | ConcreteTypeHandle.FunctionPointer concreteSig ->
            concreteSig.ParameterTypes
            |> List.map (fun p -> ConcreteSignatureType.toHandle p.UnderlyingType)
            |> shouldEqual [ int32Handle ]

            match concreteSig.ReturnType with
            | ConcreteFunctionPointerReturnType.Returns ret ->
                ConcreteSignatureType.toHandle ret.UnderlyingType |> shouldEqual int64Handle
            | ConcreteFunctionPointerReturnType.Void -> Assert.Fail "Expected Returns, got Void"
        | other -> Assert.Fail $"Expected FunctionPointer handle, got %O{other}"
    ...
    [<Test>]
    let ``CliType.zeroOf on a function pointer handle yields a null managed pointer`` () : unit =
        let signature =
            makeSignature
                [ TypeDefn.PrimitiveType PrimitiveType.Int32 ]
                (MethodReturnType.Returns (TypeDefn.PrimitiveType PrimitiveType.Int32))
        ...
        | other -> Assert.Fail $"Expected null managed-pointer zero for fnptr default, got %O{other}"
