...
// holds: an *orphaned* current directory, on a **Linux**-configured kernel.
//
...
// this file has a Darwin twin. Probed with the current directory removed out
...
class Program
{
    ...
    static unsafe int Main()
    {
        ...
        // Linux's `sys_getcwd` fails ENOENT before any length is considered;
        // Darwin's libc `getcwd` writes the root first and so needs two bytes
        // before it can start. The twin asserts the other value.
        check++;
        if (GetCwdError(1) != PAL_ENOENT) return check;
