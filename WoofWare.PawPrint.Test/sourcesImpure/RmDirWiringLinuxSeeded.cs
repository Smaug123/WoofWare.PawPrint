...
// `rmdir(2)`'s flavour-dependent facts on a **Linux**-configured kernel: which
...
class Program
{
    ...
    static unsafe int Main()
    {
        ...
        // ---- Linux specialises the path, not the root inode ----

        // Only a path that consumed no component at all is EBUSY. Darwin gives
        // this EISDIR, and gives the two below EBUSY.
        bad = Refused("/", PAL_EBUSY);
        if (bad != 0) return bad;

        // The current directory here is "/", so "." and ".." reach the root —
        // and Linux answers them exactly as it would on any other directory.
        ...
        // ENOENT for the dangling link, and — the row that matters most —
        // nothing removed through "ld/", which Darwin's walk resolves to `d`.
        ...
        // Linux reports the permission; Darwin reports the type.
