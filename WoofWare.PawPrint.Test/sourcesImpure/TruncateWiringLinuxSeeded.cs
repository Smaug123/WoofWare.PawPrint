...
// Truncation's one flavour-dependent fact, on a **Linux**-configured kernel:
...
class Program
{
    ...
    const UnixFileMode SuidAfter = UnixFileMode.UserRead | UnixFileMode.UserWrite | UnixFileMode.UserExecute
        | UnixFileMode.GroupRead | UnixFileMode.GroupExecute
        | UnixFileMode.OtherRead | UnixFileMode.OtherExecute;                          // 0o0755
    const UnixFileMode SgidAfter = SuidAfter;                                          // 0o0755
    const UnixFileMode SgidNoExecAfter = UnixFileMode.SetGroup
        | UnixFileMode.UserRead | UnixFileMode.UserWrite
        | UnixFileMode.GroupRead | UnixFileMode.OtherRead;                             // 0o2644
    const UnixFileMode StickyAfter = UnixFileMode.StickyBit | SuidAfter;               // 0o1755
    ...
    static unsafe int Main(string[] args)
    {
        ...
        // Set-group-ID *without* group-execute is the row that tells the real
        // rule from "clear both bits whenever either is set": there the bit means
        // mandatory locking rather than privilege, and Linux keeps it.
        check = 3;
        ...
        // Truncating to the length the file already has strips exactly as a real
        // one does, and leaves every byte in place. This is what separates
        // truncation from a write, where transferring nothing is a complete
        // no-op.
        ...
        // --- O_TRUNC strips the same bits ftruncate does ---
