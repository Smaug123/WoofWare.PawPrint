...
// The directory search bit, end to end, at **uid 0**, where every refusal its
// twin sees becomes a success.
//
...
// `KernelConfig.UserId`. Its twin `SearchPermissionSeeded.cs` runs the same
// checks at uid 1000 and expects EACCES from each, which is what makes the
// handler's privilege argument falsifiable in both directions: a hardcoded
// `Unprivileged` passes that file and fails this one, and a hardcoded
...
class Program
{
    ...
    static int Main()
    {
        ...
        // Root searches a directory whose mode forbids it to everyone. The
        // absent name is still absent — privilege bypasses the permission check
        // and nothing else.
        foreach (string reached in new[] { "ns/kid", "ns/f", "ns/.", "ns/..", "lns/kid" })
        {
            check++;
            if (StatPath(reached) != 0) return check;
        }

        check++;
        if (StatPath("ns/nx") != -1) return check;
        check++;
