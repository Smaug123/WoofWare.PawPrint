...
// A content-changing write's one flavour-dependent fact, on a **Darwin**-configured
...
class Program
{
    ...
    // The two rows the flavours answer differently. This file and its Linux
    // twin differ in exactly these two constants and their `KernelConfig`.
    //
    // Darwin drops the bit whatever the execute bits say, exactly as it drops
    // `S_ISUID` — where Linux would keep it, group-execute being absent.
    const UnixFileMode SgnoxAfter = Rwrr;                                              // 0o0644
    const UnixFileMode BothAfter = Rwrr;                                               // 0o0644
