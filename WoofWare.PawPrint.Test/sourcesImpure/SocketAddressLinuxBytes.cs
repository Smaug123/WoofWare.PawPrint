...
// The bytes `SocketAddressPal` lays down, under the Linux flavour. The
...
class Program
{
    ...
    static int Main(string[] args)
    {
        ...
        r = Expect(Blob(new IPEndPoint(IPAddress.Parse("1.2.3.4"), 0x1234).Serialize()),
                   new byte[] { 0x02, 0x00, 0x12, 0x34, 0x01, 0x02, 0x03, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, 10);
        if (r != 0) return r;
        ...
        r = Expect(Blob(new IPEndPoint(linkLocal, 0xABCD).Serialize()),
                   new byte[] { 0x0A, 0x00, 0xAB, 0xCD, 0x00, 0x00, 0x00, 0x00, 0xFE, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x2A, 0x00, 0x00, 0x00 }, 30);
        if (r != 0) return r;
        ...
        r = Expect(Blob(new SocketAddress(AddressFamily.InterNetwork)),
                   new byte[] { 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, 60);
        if (r != 0) return r;

        r = Expect(Blob(new SocketAddress(AddressFamily.InterNetworkV6)),
                   new byte[] { 0x0A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, 78);
        if (r != 0) return r;
        ...
        if (uds.Size != 110) return 106;
        if (uds[0] != 0x01 || uds[1] != 0x00) return 107;

        SocketAddress unspecified = new SocketAddress(AddressFamily.Unspecified);
        if (unspecified.Size != 128) return 108;
        if (unspecified[0] != 0x00 || unspecified[1] != 0) return 109;
