...
module TestSignal =
    ...
    [<Test>]
    let ``a blocked ignored signal stays pending under Linux numbering only`` () : unit =
        // Probe-pinned like the sigaction facts (measuring in-process would
        // mean changing the test host's own dispositions): SIG_IGN'd SIGUSR1
        // and default-ignored SIGWINCH, generated by kill and by
        // pthread_kill while blocked, all stayed pending on Linux 6.18.5 —
        // and were delivered to a handler installed before the unblock —
        // where Darwin 25.6.0 discarded every one at generation.
        Signal.blockedIgnoredSignalStaysPendingUnder SignalNumbering.Linux
        |> shouldEqual true

        Signal.blockedIgnoredSignalStaysPendingUnder SignalNumbering.Darwin
        |> shouldEqual false
