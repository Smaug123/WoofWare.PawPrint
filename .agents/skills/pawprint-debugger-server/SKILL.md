...
- `GET /heap?after=ADDRESS&limit=N`: list live heap objects in ascending address order, one page at a time (`limit` default 200, max 2000; pass the response's `nextAfter` as `after` for the next page, which is null on the last). Each entry has its kind, type, array length, and a string preview for strings.
- `GET /heap/{address}`: inspect an object or array at a managed heap address: its `typeDescription`, structured `fields` for an object, structured `elements` for an array, and `references`, the heap addresses it points at directly. Use structured `objectAddress` fields from stack, argument, local, and array-element values when available.
- `GET /statics`: static fields of every type that has begun initialisation, with each field's written slots (per thread for `[ThreadStatic]`) as structured values.
