...
- The suite runs on Microsoft.Testing.Platform, not VSTest. `global.json` sets `test.runner`, and the test project sets `EnableNUnitRunner`, so `NUnit3TestAdapter` is hosted *inside* `WoofWare.PawPrint.Test`'s own executable and there is no separate test-host process. Consequences for the command line: the project is named with `--project` (a positional path is rejected outright), and the VSTest-only options are gone (`--logger`, `--blame`, `--collect`) — passing one fails the run with exit code 5 rather than being ignored
- Run all tests with `nix develop -c dotnet test --project WoofWare.PawPrint.Test/WoofWare.PawPrint.Test.fsproj`
- Run a filtered subset with `nix develop -c dotnet test --project WoofWare.PawPrint.Test/WoofWare.PawPrint.Test.fsproj --no-build --filter "Name~TypeRef"`. `--filter` keeps VSTest's `TestCaseFilter` syntax, which `Microsoft.Testing.Extensions.VSTestBridge` reimplements on top of the platform, so the `Name~`/`FullyQualifiedName~` filters in the older plan docs still work
- List adapter-discovered tests with `nix develop -c dotnet test --project WoofWare.PawPrint.Test/WoofWare.PawPrint.Test.fsproj --list-tests`
- `--output Detailed` is what prints a line per test, and is the nearest equivalent of VSTest's `--logger "console;verbosity=normal"`; `--verbosity` now sets MSBuild's level, not the test reporter's. Add `--no-progress --no-ansi` when capturing to a file, or the progress line's escape sequences land in it
- The built test binary is a Microsoft.Testing.Platform application, so it also runs on its own: `nix develop -c dotnet exec WoofWare.PawPrint.Test/bin/Release/net10.0/WoofWare.PawPrint.Test.dll --filter "Name~TypeRef"`. That skips MSBuild altogether, and its `--help` is the only place the platform and extension options (as opposed to the `dotnet test` CLI's own) are listed — `--filter`, `--output`, `--settings`, `--timeout`, `--minimum-expected-tests`. `dotnet exec` is the only way in: the project sets `UseAppHost=false`, so there is no native launcher beside the dll. That is deliberate — `dotnet test` launches whatever `RunCommand` names, and an apphost is a native binary from the runtime pack that carries its own libc, which under the Nix devshell on Linux meets Nix's glibc inside one process and fails to bind CoreCLR at all. Going through the muxer keeps every route identical on every platform
- The test process runs with its GC heap capped at 50% of physical memory (`System.GC.HeapHardLimitPercent` in the test `.fsproj`, asserted by `TestGcHeapHardLimit.fs`). Unbounded, the suite's peak RSS exceeds what a 16 GB CI runner has, and the OOM killer reaps it mid-run — which reports nothing about which test was running, or that memory was involved at all. If you see that shape of failure, suspect memory rather than the diff under test
...
* Never run the test suite as `dotnet test ... | grep ... | head -N` in the background: `head` closes the pipe once it has N lines, so the reported exit code is *head's* (always 0), and the `Test run summary:` block — emitted last — gets cut off entirely. A run that looks green may have failed. Capture output directly to a file with no pipeline, then grep the file afterwards for `"Test run summary|^ *(total|failed|succeeded|skipped):"`.
