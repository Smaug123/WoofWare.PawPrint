...
- `docs/plans/2026-08-23-posix-kernel-extraction.md` is the plan, including the stages not yet done and the decisions behind the split. `scripts/check-move-is-rename-only.sh` is the oracle the move stages are held to, and `scripts/check-docstring-attachment.py` is its companion: an F# `///` block binds to the declaration that *follows* it, so moving a definition without its docstring silently re-binds the prose to whatever now follows, invisibly to the compiler, the tests and the diff. Run it against the branch point for any commit that moves definitions between files — and note that it also catches the other way prose comes adrift, which is *inserting* a declaration between a docstring and its subject rather than moving anything. `scripts/test-docstring-attachment.sh` is that check's own executable contract: fifteen shapes, seven of which must report and eight of which must stay silent
...
- The seven fixtures that run guests under the interpreter — `TestPureCases`, `TestImpureCases`, `TestSignalTerminatedCases`, `TestFSharpPureCases`, `TestClockJitterFindsBugs`, `TestInlineArrayLayout`, `TestScheduleFork` — are `[<Category("Guest")>]` and `[<Explicit>]`. They are 92% of the suite's cost and 23% of its tests, so a bare `dotnet test` skips them: the default run is ~3450 tests in about 80 seconds instead of ~4450 in about 17 minutes. **A change that touches the interpreter is not tested until you have run the Guest fixtures too.** CI runs both halves (see `.github/workflows/ci.yaml`); nothing is excluded from a PR
