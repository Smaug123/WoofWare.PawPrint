...
- Rules of the CLI execution model, kept as inspectable data rather than as control flow inside the interpreter. `ContextSwitchPrior.fs` (how likely interleaving an opcode is to matter, consumed by the PCT scheduler) and `OpcodeFaults.fs` (which exceptions an instruction raises by itself, as opposed to what reaches it from a callee) are the residents
...
- That intent is **not yet met**, and the gap is known rather than latent: seven definitions still speak CoreCLR's PAL encodings rather than POSIX ones, listed in `scripts/pal-residue-allowlist.txt` (the errno cluster left in stage 7 and the socket-event bit mask in stage 9g; what remains is the socket *creation* vocabulary, which goes with the socket entry points' own moves, and the managed-enum signal mapping). They are leaf adapters consumed almost entirely by `Native/NativeSystemNative.fs`, and belong on the PawPrint side of the boundary. Do not add more — a new PAL encoding goes in the native-call adapter, and the library gets the POSIX value. `scripts/check-pal-residue.py` enforces that as the `pal-residue` flake check: the allowlist may shrink, never grow, and a definition is caught by its name or by its body mentioning a PAL encoding
