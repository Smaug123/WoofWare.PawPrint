...
- That intent is **not yet met**, and the gap is known rather than latent: eleven functions still speak CoreCLR's PAL encodings rather than POSIX ones — `UnixError.toPal`/`palOfRawErrno`/`palOfRawErrnoUnder`, `Signal.ofPosixSignalEnum`/`toPosixSignalEnum`, `SocketEventInterest.ofBits`, `PollEvents.ofBits`/`toBits`, `SimulatedUnixPlatform.addressFamilyPalToPlatform`/`addressFamilyPlatformToPal`/`socketCreation`. They are leaf adapters consumed almost entirely by `Native/NativeSystemNative.fs`, and belong on the PawPrint side of the boundary. Do not add more: a new PAL encoding goes in the native-call adapter, and the library gets the POSIX value
