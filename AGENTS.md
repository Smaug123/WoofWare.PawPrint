...
- That intent is **not yet met**, and the gap is known rather than latent: two definitions still speak CoreCLR's PAL encodings rather than POSIX ones, listed in `scripts/pal-residue-allowlist.txt` (the errno cluster left in stage 7, the socket-event bit mask in stage 9g, and the socket-creation numbering in stage 9h; what remains is the managed-enum signal mapping). They are leaf adapters consumed almost entirely by `Native/NativeSystemNative.fs`, and belong on the PawPrint side of the boundary. Do not add more — a new PAL encoding goes in the native-call adapter, and the library gets the POSIX value. `scripts/check-pal-residue.py` enforces that as the `pal-residue` flake check: the allowlist may shrink, never grow, and a definition is caught by its name or by its body mentioning a PAL encoding
