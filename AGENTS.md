...
- The six fixtures that run guests under the interpreter — `TestPureCases`, `TestImpureCases`, `TestFSharpPureCases`, `TestClockJitterFindsBugs`, `TestInlineArrayLayout`, `TestScheduleFork` — are `[<Category("Guest")>]` and `[<Explicit>]`. They are 92% of the suite's cost and 23% of its tests, so a bare `dotnet test` skips them: the default run is ~3450 tests in about 80 seconds instead of ~4450 in about 17 minutes. **A change that touches the interpreter is not tested until you have run the Guest fixtures too.** CI runs both halves (see `.github/workflows/ci.yaml`); nothing is excluded from a PR
